...
; (function ($) {
    ...
    $.fn.bcalendar = function (option) {
        ...
        //日期和月的函数
        var __SCOLLEVENTTEMP = "<DIV style=\"WIDTH:${width};top:${top};left:${left};\" title=\"${title}\" class=\"chip chip${i} ${drag}\"><div class=\"dhdV\" style=\"display:none\">${data}</div><DIV style=\"BORDER-BOTTOM-COLOR:${bdcolor}\" class=ct>&nbsp;</DIV><DL style=\"BORDER-BOTTOM-COLOR:${bdcolor}; BACKGROUND-COLOR:${bgcolor1}; BORDER-TOP-COLOR: ${bdcolor}; HEIGHT: ${height}px; BORDER-RIGHT-COLOR:${bdcolor}; BORDER-LEFT-COLOR:${bdcolor}\"><DT style=\"BACKGROUND-COLOR:${bgcolor2}\">${starttime} – ${endtime} ${icon}</DT><DD><SPAN>${content}</SPAN></DD><DD><SPAN>${remark}</SPAN></DD><DIV class='resizer' style='display:${redisplay}'><DIV class=rszr_icon>&nbsp;</DIV></DIV></DL><DIV style=\"BORDER-BOTTOM-COLOR:${bdcolor}; BACKGROUND-COLOR:${bgcolor1}; BORDER-TOP-COLOR: ${bdcolor}; BORDER-RIGHT-COLOR: ${bdcolor}; BORDER-LEFT-COLOR:${bdcolor}\" class=cb1>&nbsp;</DIV><DIV style=\"BORDER-BOTTOM-COLOR:${bdcolor}; BORDER-TOP-COLOR:${bdcolor}; BORDER-RIGHT-COLOR:${bdcolor}; BORDER-LEFT-COLOR:${bdcolor}\" class=cb2>&nbsp;</DIV></DIV>";
        ...
        function BuildDaysAndWeekView(startday, l, events, config) {
            var days = [];
            var show;
            var alheight = 42 * (option.etime - option.stime + 1);
            if (l == 1) {
                show = dateFormat.call(startday, i18n.xgcalendar.dateformat.Md);
        ...
        function PropareEvents(dayarrs, events, aDE, sDE) {
            ...
            for (var j = 0; j < el; j++) {
                ...
                //时间限制
                s.st.p = (s.st.hour - option.stime) * 60 + s.st.minute; // 时间的开始位置
                s.et = {};
                s.et.hour = eD.getHours();
                s.et.minute = eD.getMinutes();
                s.et.p = (s.et.hour - option.stime) * 60 + s.et.minute; // 时间的结束位置
                fE.push(s);
            }
            var dMax = 0;
            for (var i = 0; i < l; i++) {
        ...
        function BuildDayScollEventContainer(ht, dayarrs, events) {
            ...
            for (var i = option.stime; i <= option.etime; i++) {
            ...
            var mHg;
            if (h >= option.stime && h <= option.etime) {
                mHg = gP(h, m) - 4; //减去标志本身一半的高度，可以让箭头刚好指向目标位置
            ...
            for (var i = 0; i < l; i++) {
                ...
                // Today
                if (istoday) {
                    ht.push("<div style=\"margin-bottom: -", alheight, "px; height:", alheight, "px\" class=\"tg-today\">&nbsp;</div>");
                }
                //var eventC = $(eventWrap);
                //onclick=\"javascript:FunProxy('rowhandler',event,this);\"
                ht.push("<div  style=\"margin-bottom: -", alheight, "px; height: ", alheight, "px\" id='tgCol", i, "' class=\"tg-col-eventwrapper\">");
                BuildEvents(ht, events[i], dayarrs[i]);
                ht.push("</div>");

                ht.push("<div class=\"tg-col-overlaywrapper\" id='tgOver", i, "'>");
                if (istoday && h >= option.stime && h <= option.etime) {
                    var mhh = mHg + 4;
        ...
        function gP(h, m) {
            return (h - option.stime) * 42 + parseInt(m / 60 * 42);
        }
        function gW(ts1, ts2) {
            var t1 = ts1 / 42;
        ...
        function quickadd(start, end, isallday, pos) {
            ...
            if (buddle.length == 0) {
                ...
                temparr.push('<td class="bubble-cell-main"><div class="bubble-top"></div><td class="bubble-cell-side"><div id="tr1" class="bubble-corner"><div class="bubble-sprite bubble-tr"></div></div>  <tr><td class="bubble-mid" colSpan="3"><div id="bubbleContent1"><div><div></div><div class="cb-root">');
                ...
                temparr.push(i18n.xgcalendar.content, ':</th><td class="cb-value"><div class="textbox-fill-wrapper"><div class="textbox-fill-mid"><select id="bbit-cal-what" class="ui-select" style="width:100%"/></div></div>');//<div class="cb-example">
                temparr.push(/*i18n.xgcalendar.example, */'</div></td></tr></tbody></table><input id="bbit-cal-start" type="hidden"/><input id="bbit-cal-end" type="hidden"/><input id="bbit-cal-allday" type="hidden"/><input id="bbit-cal-quickAddBTN" class value="');
                ...
                calbutton.click(function (e) {
                    ...
                    option.isloading = true;
                    var code = $("#bbit-cal-what").val();
                    var what = option.ClickCmdhandler ? $("#bbit-cal-what option:selected").text() : $("#bbit-cal-what").val();
                    ...
                    var zone = new Date().getTimezoneOffset() / 60 * -1;
                    var param = [{ "name": "CalendarCode", value: code },
                    ...
                    if (option.quickAddHandler && $.isFunction(option.quickAddHandler)) {
                        ...
                        newdata.push(parseInt(code.replace(/[^0-9]/ig, "")), 0, "", ""); //主题,权限,参与人，
                ...
                //會議室選擇
                if (option.ClickCmdhandler) {
                    if (option.ClickCmdhandler && $.isFunction(option.ClickCmdhandler)) {
                        option.ClickCmdhandler.call();
                        //$("#bbit-cal-what").removeClass();
                    }
                } else {
                    $("#bbit-cal-what").removeClass().addClass("textbox-fill-input");
                }
