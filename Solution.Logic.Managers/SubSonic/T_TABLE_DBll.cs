...
namespace Solution.Logic.Managers {
	...
	public partial class T_TABLE_DBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_T_TABLE_D";
        private const string const_CacheKey_Date = "Cache_T_TABLE_D_Date";

		#region 單例模式
		//定義單例實伐
		private static T_TABLE_DBll _T_TABLE_DBll = null;

		/// <summary>
		/// 獲取本邏輯類單例
		/// </summary>
		/// <returns></returns>
		public static T_TABLE_DBll GetInstence() {
			if (_T_TABLE_DBll == null) {
				_T_TABLE_DBll = new T_TABLE_DBll();
			}
			return _T_TABLE_DBll;
		}
		#endregion
		...
		#region 從IIS緩存中獲取T_TABLE_D表記錄
		/// <summary>
        /// 從IIS緩存中獲取T_TABLE_D表記錄
        /// </summary>
	    /// <param name="isCache">是否從緩存中讀取</param>
        public IList<DataAccess.Model.T_TABLE_D> GetList(bool isCache = true)
        {
			try
			{
				//判斷是否使用緩存
				if (CommonBll.IsUseCache() && isCache){
					...
					{
						return (IList<DataAccess.Model.T_TABLE_D>)obj;
					}
				}
				else
				{
					//定義臨時實伐集
					IList<DataAccess.Model.T_TABLE_D> list = null;

					//獲??表緩存加載條件表達式
					var exp = GetExpression<T_TABLE_D>();
                    //?果條件為空，則查詢?表所有記錄
					if (exp == null)
					{
						//從數據庫中獲?所有記錄
						var all = T_TABLE_D.All();
                        list = all == null ? null : Transform(all.ToList());
					}
					else
					{
                        //從數據庫中查詢出指定條件的記錄，並轉換為指定實伐集
						var all = T_TABLE_D.Find(exp);
            ...
            {
                //記錄?志
                CommonBll.WriteLog("從IIS緩存中獲取T_TABLE_D表記錄時出現異常", e);
			}
		...
		/// <returns>DataAccess.Model.T_TABLE_D</returns>
        public DataAccess.Model.T_TABLE_D GetModel(long id, bool isCache = true)
        {
            //判斷是否使用緩存
		    if (CommonBll.IsUseCache() && isCache)
		    {
		    ...
		    {
                //從數據庫中直接讀?
                var model = T_TABLE_D.SingleOrDefault(x => x.Id == id);
                if (model == null)
                {
                    return null;
                }
        ...
        /// <param name="id">主鍵Id</param>
		/// <returns>DataAccess.Model.T_TABLE_D</returns>
        public DataAccess.Model.T_TABLE_D GetModelForCache(long id)
        {
			try
			{
				//從緩存中讀?指定Id記錄
				...
				if (model == null){
					//從數據庫中讀?
					var tem = T_TABLE_D.SingleOrDefault(x => x.Id == id);
					if (tem == null)
					{
						return null;
					}
        ...
        /// <returns>DataAccess.Model.T_TABLE_D</returns>
        public DataAccess.Model.T_TABLE_D GetModelForCache(string conditionColName, object value)
        {
			try
            {
                //從緩存中獲取List
                var list = GetList();
                DataAccess.Model.T_TABLE_D model = null;
                Expression<Func<T_TABLE_D, bool>> expression = null;
                ...
                switch (conditionColName)
                {
					case "Id" :
                        ...
                        break;
					case "TABLES" :
						model = list.SingleOrDefault(x => x.TABLES == (string)value);
                        expression = x => x.TABLES == (string)value;
                        break;
					case "CODE" :
                        ...
                        break;
					case "DESCR" :
						model = list.SingleOrDefault(x => x.DESCR == (string)value);
                        expression = x => x.DESCR == (string)value;
                        break;
					case "DATA" :
						model = list.SingleOrDefault(x => x.DATA == (string)value);
                        expression = x => x.DATA == (string)value;
                        break;
					case "TYPE" :
					...
					case "GROUPS" :
                        ...
                        break;
					case "DATAC" :
						model = list.SingleOrDefault(x => x.DATAC == (int)value);
                        expression = x => x.DATAC == (int)value;
                        break;
					case "NOTES" :
					...
					case "RECORD_DATE" :
                        ...
                        break;
					case "SYSDIV" :
						model = list.SingleOrDefault(x => x.SYSDIV == (string)value);
                        expression = x => x.SYSDIV == (string)value;
                        break;
					case "DATAD1" :
						model = list.SingleOrDefault(x => x.DATAD1 == (decimal)value);
                        expression = x => x.DATAD1 == (decimal)value;
                        break;
					case "DATAD2" :
						model = list.SingleOrDefault(x => x.DATAD2 == (decimal)value);
                        expression = x => x.DATAD2 == (decimal)value;
                        break;
					case "DATAD3" :
						model = list.SingleOrDefault(x => x.DATAD3 == (decimal)value);
                        expression = x => x.DATAD3 == (decimal)value;
                        break;
					case "DATAD4" :
						model = list.SingleOrDefault(x => x.DATAD4 == (decimal)value);
                        expression = x => x.DATAD4 == (decimal)value;
                        break;
					case "DATAD5" :
						model = list.SingleOrDefault(x => x.DATAD5 == (string)value);
                        expression = x => x.DATAD5 == (string)value;
                        break;
					case "DATAD6" :
						model = list.SingleOrDefault(x => x.DATAD6 == (string)value);
                        expression = x => x.DATAD6 == (string)value;
                        break;
					case "DATAD7" :
						model = list.SingleOrDefault(x => x.DATAD7 == (string)value);
                        expression = x => x.DATAD7 == (string)value;
                        break;
					case "DATAD8" :
						model = list.SingleOrDefault(x => x.DATAD8 == (string)value);
                        expression = x => x.DATAD8 == (string)value;
                        break;
					case "EDIT_BY" :
                ...
                {
                    //從數據庫中讀取
                    var tem = T_TABLE_D.SingleOrDefault(expression);
                    if (tem == null)
                    {
                        return null;
                    }
        ...
        /// <returns>DataAccess.Model.T_TABLE_D</returns>
        public DataAccess.Model.T_TABLE_D GetModelForCache(Expression<Func<DataAccess.Model.T_TABLE_D, bool>> expression)
        {
			//從緩存中讀?記錄列表
			var list = GetList();
            //?果條件為空，則查詢?表所有記錄
        ...
        public void SetModelForCache(DataAccess.Model.T_TABLE_D model)
        {
		    ...
		    if (list == null)
		    {
                list = new List<DataAccess.Model.T_TABLE_D>();
		    }
            //添加記錄
            list.Add(model);
        }
        ...
        /// </summary>
        /// <param name="model">記錄實伐</param>
        public void SetModelForCache(T_TABLE_D model)
        {
		...
		/// <summary>
        /// 按條件刪除IIS緩存中T_TABLE_D表的指定記錄
        /// </summary>
        /// <param name="expression">條件，值為null時刪除?有記錄</param>
		public void DelCache(Expression<Func<DataAccess.Model.T_TABLE_D, bool>> expression)
        {
            //從緩存中獲?List
		    var list = GetList();
            //?果緩存為null，則不做?何處理
		...
		#region 實伐轉換
		/// <summary>
		/// 將T_TABLE_D記錄實伐（SubSonic實伐）轉換為?通的實伐（DataAccess.Model.T_TABLE_D）
		/// </summary>
        /// <param name="model">SubSonic插件生成的實伐</param>
		/// <returns>DataAccess.Model.T_TABLE_D</returns>
		public DataAccess.Model.T_TABLE_D Transform(T_TABLE_D model)
        {			
			if (model == null) 
				return null;

            return new DataAccess.Model.T_TABLE_D
            {
                Id = model.Id,
                TABLES = model.TABLES,
                CODE = model.CODE,
		...
		/// <summary>
		/// 將T_TABLE_D記錄實伐集（SubSonic實伐）轉換為?通的實伐集（DataAccess.Model.T_TABLE_D）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的實伐集</param>
        public IList<DataAccess.Model.T_TABLE_D> Transform(IList<T_TABLE_D> sourceList)
        {
			//創建List??
            var list = new List<DataAccess.Model.T_TABLE_D>();
		...
		/// <summary>
		/// 將T_TABLE_D記錄實伐批?通的實伐（DataAccess.Model.T_TABLE_D）轉換為SubSonic插件生成的實伐
		/// </summary>
        /// <param name="model">?通的實伐（DataAccess.Model.T_TABLE_D）</param>
		/// <returns>T_TABLE_D</returns>
		public T_TABLE_D Transform(DataAccess.Model.T_TABLE_D model)
        {
			if (model == null) 
				return null;

            return new T_TABLE_D
            {
                Id = model.Id,
                TABLES = model.TABLES,
                CODE = model.CODE,
		...
		/// <summary>
		/// 將T_TABLE_D記錄實伐批?通實伐集（DataAccess.Model.T_TABLE_D）轉換為SubSonic插件生成的實伐集
		/// </summary>
        /// <param name="sourceList">?通實伐集（DataAccess.Model.T_TABLE_D）</param>
        public IList<T_TABLE_D> Transform(IList<DataAccess.Model.T_TABLE_D> sourceList)
        {
			//創建List??
            var list = new List<T_TABLE_D>();
		...
		public void SetModelValue(DataAccess.Model.T_TABLE_D model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.T_TABLE_D model, string colName, object value)
		{
            ...
            switch (colName)
            {
				case "Id" :
                    ...
                    break;
				case "TABLES" :
					model.TABLES = (string)value;
                    break;
				case "CODE" :
					model.CODE = (string)value;
                    break;
				case "DESCR" :
					model.DESCR = (string)value;
                    break;
				case "DATA" :
					model.DATA = (string)value;
                    break;
				case "TYPE" :
				...
				case "RECORD_DATE" :
                    ...
                    break;
				case "SYSDIV" :
					model.SYSDIV = (string)value;
                    break;
				case "DATAD1" :
					model.DATAD1 = (decimal)value;
                    break;
				case "DATAD2" :
					model.DATAD2 = (decimal)value;
                    break;
				case "DATAD3" :
					model.DATAD3 = (decimal)value;
                    break;
				case "DATAD4" :
					model.DATAD4 = (decimal)value;
                    break;
				case "DATAD5" :
					model.DATAD5 = (string)value;
                    break;
				case "DATAD6" :
					model.DATAD6 = (string)value;
                    break;
				case "DATAD7" :
					model.DATAD7 = (string)value;
                    break;
				case "DATAD8" :
					model.DATAD8 = (string)value;
                    break;
				case "EDIT_BY" :
        ...
        public int GetRecordCount()
        {
			...
			else
			{
				...
				return select.GetRecordCount<T_TABLE_D>();
			}
        }

		/// <summary>
		/// 獲?T_TABLE_D表記錄總數——從數據庫中查詢
		/// </summary>
        /// <param name="wheres">條件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionHelper.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<T_TABLE_D>(wheres);

		}

		/// <summary>
		/// 獲?T_TABLE_D表指定條件的記錄總數——從數據庫中查詢
		/// </summary>
        /// <param name="expression">條件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<T_TABLE_D, bool>> expression) {
            return new Select().From<T_TABLE_D>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.T_TABLE_D> Find(Expression<Func<DataAccess.Model.T_TABLE_D, bool>> expression)
        {
			//從緩存中獲?記錄集
			var list = GetList();
            //判斷獲?記錄集是否為null
        ...
        public bool Exist(int id)
        {
            ...
            //從數據庫中查找
            return T_TABLE_D.Exists(x => x.Id == id);
        }
        ...
        /// <param name="expression">條件語句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.T_TABLE_D, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionHelper.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分頁查詢
                var select = new SelectHelper();
                return select.SelectDataTable<T_TABLE_D>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //記錄?志
                CommonBll.WriteLog("獲?T_TABLE_D表記錄時出現異常", e);
		...
		#region 添加與編輯T_TABLE_D表記錄
		/// <summary>
		/// 添加與編輯T_TABLE_D記錄
		/// </summary>
	    /// <param name="page">當?頁面指針</param>
		/// <param name="model">T_TABLE_D表實伐</param>
        /// <param name="content">更新說明</param>
        /// <param name="isCache">是否更新緩存</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
        public void Save(Page page, T_TABLE_D model, string content = null, bool isCache = true, bool isAddUseLog = true)
        {
			try {
				//保存
				model.Save();
				...
				if (isAddUseLog)
				{
					...
					{
						content = "{0}" + (model.Id == 0 ? "添加" : "編輯") + "T_TABLE_D記錄成功，ID為【" + model.Id + "】";
					}
				...
				var result = "執行T_TABLE_DBll.Save()函數出錯！";
		...
		{
			//設瞞Sql語句
			var sql = string.Format("delete from {0} where {1} = {2}", T_TABLE_DTable.TableName,  T_TABLE_DTable.Id, id);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了T_TABLE_D表id為【" + id + "】的記錄！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//設瞞Sql語句
			var sql = string.Format("delete from {0} where {1} in ({2})", T_TABLE_DTable.TableName,  T_TABLE_DTable.Id, str);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了T_TABLE_D表id為【" + str + "】的記錄！");
			}
		}

		/// <summary>
        /// 刪除T_TABLE_D表記錄——?果使用了緩存，刪除成功後會?空本表的所有緩存記錄，?後重新加載進緩存
        /// </summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="expression">條件語句</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
        public void Delete(Page page, Expression<Func<T_TABLE_D, bool>> expression, bool isAddUseLog = true)
        {
			//執行刪除
			T_TABLE_D.Delete(expression);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了T_TABLE_D表記錄！");
			}
        }
        ...
        {
            //設瞞Sql語句
            var sql = string.Format("delete from {0}", T_TABLE_DTable.TableName);
            ...
            {
                //添加用戶操作記錄
                UseLogBll.GetInstence().Save(page, "{0}刪除了T_TABLE_D表所有記錄！");
            }
        }
		#endregion
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "T_TABLE_D", sortName, "Id"))
		    {
				//判斷是否?用緩存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}更新了T_TABLE_D表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "T_TABLE_D", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判斷是否?用緩存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}對T_TABLE_D表進行了自動排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
	        ...
	        else
	        {
				if (string.IsNullOrEmpty(conditionColName))
	            {
	                conditionColName = T_TABLE_DTable.Id;
	            }
        ...
        /// <param name="expression">條件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.T_TABLE_D, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<T_TABLE_D>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.T_TABLE_D model, string colName)
		{
		...
		#region 更新T_TABLE_D表指定字段值
		/// <summary>更新T_TABLE_D表記錄指定字段值，?果使用了緩存，保存成功後會?空本表的所有緩存記錄，?後重新加載進緩存</summary>
		/// <param name="page">當?頁面指針</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionHelper.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<T_TABLE_D>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用戶操作記錄
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}囊改了T_TABLE_D表記錄。");				
				}
				else
				{
					//添加用戶操作記錄
		...
		#region 更新T_TABLE_D表指定主鍵Id的字段值
		/// <summary>更新T_TABLE_D表記錄指定字段值</summary>
        /// <param name="page">當?頁面指針</param>
        ...
        {
			content = content != "" ? content : "{0}囊改了T_TABLE_D表主鍵Id值為" + id + "的記錄。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionHelper.SqlqueryCondition(ConstraintType.And, T_TABLE_DTable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新T_TABLE_D表記錄指定字段值（更新一個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了T_TABLE_D表主鍵Id值為" + id + "的記錄，將" + columnName + "字段值囊改為" + columnValue;
            //設瞞更新字段
		 ...
		 /// <summary>更新T_TABLE_D表記錄指定字段值（更新兩個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了T_TABLE_D表主鍵Id值為" + id + "的記錄，將" + columnName1 + "字段值囊改為" + columnValue1 + "，" + columnName2 + "字段值囊改為" + columnValue2;
            //設瞞更新字段
            var dic = new Dictionary<string, object>();
