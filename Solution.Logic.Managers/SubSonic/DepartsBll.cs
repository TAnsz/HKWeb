...
namespace Solution.Logic.Managers {
	...
	public partial class DepartsBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_Departs";
        private const string const_CacheKey_Date = "Cache_Departs_Date";

		#region 單例模式
		//定義單例實伐
		private static DepartsBll _DepartsBll = null;

		/// <summary>
		/// 獲取本邏輯類單例
		/// </summary>
		/// <returns></returns>
		public static DepartsBll GetInstence() {
			if (_DepartsBll == null) {
				_DepartsBll = new DepartsBll();
			}
			return _DepartsBll;
		}
		#endregion
		...
		#region 從IIS緩存中獲取Departs表記錄
		/// <summary>
        /// 從IIS緩存中獲取Departs表記錄
        /// </summary>
	    /// <param name="isCache">是否從緩存中讀取</param>
        public IList<DataAccess.Model.Departs> GetList(bool isCache = true)
        {
			try
			{
				//判斷是否使用緩存
				if (CommonBll.IsUseCache() && isCache){
					...
					{
						return (IList<DataAccess.Model.Departs>)obj;
					}
				}
				else
				{
					//定義臨時實伐集
					IList<DataAccess.Model.Departs> list = null;

					//獲??表緩存加載條件表達式
					var exp = GetExpression<Departs>();
                    //?果條件為空，則查詢?表所有記錄
					if (exp == null)
					{
						//從數據庫中獲?所有記錄
            ...
            {
                //記錄?志
                CommonBll.WriteLog("從IIS緩存中獲取Departs表記錄時出現異常", e);
			}
		...
		/// <returns>DataAccess.Model.Departs</returns>
        public DataAccess.Model.Departs GetModel(long id, bool isCache = true)
        {
            //判斷是否使用緩存
		    if (CommonBll.IsUseCache() && isCache)
		    {
		    ...
		    {
                //從數據庫中直接讀?
                var model = Departs.SingleOrDefault(x => x.Id == id);
                if (model == null)
                {
                    return null;
                }
        ...
        /// <param name="id">主鍵Id</param>
		/// <returns>DataAccess.Model.Departs</returns>
        public DataAccess.Model.Departs GetModelForCache(long id)
        {
			try
			{
				//從緩存中讀?指定Id記錄
				...
				if (model == null){
					//從數據庫中讀?
					var tem = Departs.SingleOrDefault(x => x.Id == id);
					if (tem == null)
					{
						return null;
					}
        ...
        /// <returns>DataAccess.Model.Departs</returns>
        public DataAccess.Model.Departs GetModelForCache(string conditionColName, object value)
        {
			try
            {
                //從緩存中獲取List
                var list = GetList();
                DataAccess.Model.Departs model = null;
                Expression<Func<Departs, bool>> expression = null;
                ...
                switch (conditionColName)
                {
					...
					case "depart_id" :
                        ...
                        break;
					case "inside_id" :
						model = list.SingleOrDefault(x => x.inside_id == (int)value);
                        expression = x => x.inside_id == (int)value;
                        break;
					case "depart_name" :
						model = list.SingleOrDefault(x => x.depart_name == (string)value);
                        expression = x => x.depart_name == (string)value;
                        break;
					case "manager1" :
						model = list.SingleOrDefault(x => x.manager1 == (string)value);
                        expression = x => x.manager1 == (string)value;
                        break;
					case "job_id1" :
						model = list.SingleOrDefault(x => x.job_id1 == (string)value);
                        expression = x => x.job_id1 == (string)value;
                        break;
					case "manager2" :
						model = list.SingleOrDefault(x => x.manager2 == (string)value);
                        expression = x => x.manager2 == (string)value;
                        break;
					case "job_id2" :
						model = list.SingleOrDefault(x => x.job_id2 == (string)value);
                        expression = x => x.job_id2 == (string)value;
                        break;
					case "emp_prefix" :
						model = list.SingleOrDefault(x => x.emp_prefix == (string)value);
                        expression = x => x.emp_prefix == (string)value;
                        break;
					case "plan_num" :
						model = list.SingleOrDefault(x => x.plan_num == (int)value);
                        expression = x => x.plan_num == (int)value;
                        break;
					case "linktel" :
						model = list.SingleOrDefault(x => x.linktel == (string)value);
                        expression = x => x.linktel == (string)value;
                        break;
					case "remark" :
                        ...
                        break;
					case "butei_money" :
						model = list.SingleOrDefault(x => x.butei_money == (decimal)value);
                        expression = x => x.butei_money == (decimal)value;
                        break;
					case "audit_access" :
						model = list.SingleOrDefault(x => x.audit_access == (int)value);
                        expression = x => x.audit_access == (int)value;
                        break;
					case "TreeLevel" :
						model = list.SingleOrDefault(x => x.TreeLevel == (int)value);
                        expression = x => x.TreeLevel == (int)value;
                        break;
					case "Up_ID" :
						model = list.SingleOrDefault(x => x.Up_ID == (string)value);
                        expression = x => x.Up_ID == (string)value;
                        break;
					case "LongName" :
						model = list.SingleOrDefault(x => x.LongName == (string)value);
                        expression = x => x.LongName == (string)value;
                        break;
					case "access_level" :
                ...
                {
                    //從數據庫中讀取
                    var tem = Departs.SingleOrDefault(expression);
                    if (tem == null)
                    {
                        return null;
                    }
        ...
        /// <returns>DataAccess.Model.Departs</returns>
        public DataAccess.Model.Departs GetModelForCache(Expression<Func<DataAccess.Model.Departs, bool>> expression)
        {
			//從緩存中讀?記錄列表
			var list = GetList();
            //?果條件為空，則查詢?表所有記錄
        ...
        public void SetModelForCache(DataAccess.Model.Departs model)
        {
		    ...
		    if (list == null)
		    {
                list = new List<DataAccess.Model.Departs>();
		    }
            //添加記錄
            list.Add(model);
        }
        ...
        /// </summary>
        /// <param name="model">記錄實伐</param>
        public void SetModelForCache(Departs model)
        {
		...
		/// <summary>
        /// 按條件刪除IIS緩存中Departs表的指定記錄
        /// </summary>
        /// <param name="expression">條件，值為null時刪除?有記錄</param>
		public void DelCache(Expression<Func<DataAccess.Model.Departs, bool>> expression)
        {
            //從緩存中獲?List
		    var list = GetList();
            //?果緩存為null，則不做?何處理
		...
		#region 實伐轉換
		/// <summary>
		/// 將Departs記錄實伐（SubSonic實伐）轉換為?通的實伐（DataAccess.Model.Departs）
		/// </summary>
        /// <param name="model">SubSonic插件生成的實伐</param>
		/// <returns>DataAccess.Model.Departs</returns>
		public DataAccess.Model.Departs Transform(Departs model)
        {			
			if (model == null) 
				return null;

            return new DataAccess.Model.Departs
            {
                Id = model.Id,
                depart_id = model.depart_id,
                inside_id = model.inside_id,
                depart_name = model.depart_name,
                manager1 = model.manager1,
                job_id1 = model.job_id1,
                manager2 = model.manager2,
                job_id2 = model.job_id2,
                emp_prefix = model.emp_prefix,
                plan_num = model.plan_num,
                linktel = model.linktel,
                remark = model.remark,
                butei_money = model.butei_money,
                audit_access = model.audit_access,
		...
		/// <summary>
		/// 將Departs記錄實伐集（SubSonic實伐）轉換為?通的實伐集（DataAccess.Model.Departs）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的實伐集</param>
        public IList<DataAccess.Model.Departs> Transform(IList<Departs> sourceList)
        {
			//創建List??
            var list = new List<DataAccess.Model.Departs>();
		...
		/// <summary>
		/// 將Departs記錄實伐批?通的實伐（DataAccess.Model.Departs）轉換為SubSonic插件生成的實伐
		/// </summary>
        /// <param name="model">?通的實伐（DataAccess.Model.Departs）</param>
		/// <returns>Departs</returns>
		public Departs Transform(DataAccess.Model.Departs model)
        {
			if (model == null) 
				return null;
		...
		/// <summary>
		/// 將Departs記錄實伐批?通實伐集（DataAccess.Model.Departs）轉換為SubSonic插件生成的實伐集
		/// </summary>
        /// <param name="sourceList">?通實伐集（DataAccess.Model.Departs）</param>
        public IList<Departs> Transform(IList<DataAccess.Model.Departs> sourceList)
        {
			//創建List??
            var list = new List<Departs>();
		...
		public void SetModelValue(DataAccess.Model.Departs model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.Departs model, string colName, object value)
		{
            ...
            switch (colName)
            {
				...
				case "depart_id" :
                    ...
                    break;
				case "inside_id" :
					model.inside_id = (int)value;
                    break;
				case "depart_name" :
					model.depart_name = (string)value;
                    break;
				case "manager1" :
					model.manager1 = (string)value;
                    break;
				case "job_id1" :
					model.job_id1 = (string)value;
                    break;
				case "manager2" :
					model.manager2 = (string)value;
                    break;
				case "job_id2" :
					model.job_id2 = (string)value;
                    break;
				case "emp_prefix" :
					model.emp_prefix = (string)value;
                    break;
				case "plan_num" :
					model.plan_num = (int)value;
                    break;
				case "linktel" :
					model.linktel = (string)value;
                    break;
				case "remark" :
					model.remark = (string)value;
                    break;
				case "butei_money" :
					model.butei_money = (decimal)value;
                    break;
				case "audit_access" :
					model.audit_access = (int)value;
                    break;
				case "TreeLevel" :
					model.TreeLevel = (int)value;
                    break;
				case "Up_ID" :
					model.Up_ID = (string)value;
                    break;
				case "LongName" :
					model.LongName = (string)value;
                    break;
				case "access_level" :
        ...
        public int GetRecordCount()
        {
			...
			else
			{
				...
				return select.GetRecordCount<Departs>();
			}
        }

		/// <summary>
		/// 獲?Departs表記錄總數——從數據庫中查詢
		/// </summary>
        /// <param name="wheres">條件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionHelper.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<Departs>(wheres);

		}

		/// <summary>
		/// 獲?Departs表指定條件的記錄總數——從數據庫中查詢
		/// </summary>
        /// <param name="expression">條件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<Departs, bool>> expression) {
            return new Select().From<Departs>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.Departs> Find(Expression<Func<DataAccess.Model.Departs, bool>> expression)
        {
			//從緩存中獲?記錄集
			var list = GetList();
            //判斷獲?記錄集是否為null
        ...
        public bool Exist(int id)
        {
            ...
            //從數據庫中查找
            return Departs.Exists(x => x.Id == id);
        }
        ...
        /// <param name="expression">條件語句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.Departs, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionHelper.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分頁查詢
                var select = new SelectHelper();
                return select.SelectDataTable<Departs>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //記錄?志
                CommonBll.WriteLog("獲?Departs表記錄時出現異常", e);
		...
		#region 添加與編輯Departs表記錄
		/// <summary>
		/// 添加與編輯Departs記錄
		/// </summary>
	    /// <param name="page">當?頁面指針</param>
		/// <param name="model">Departs表實伐</param>
        /// <param name="content">更新說明</param>
        /// <param name="isCache">是否更新緩存</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
        public void Save(Page page, Departs model, string content = null, bool isCache = true, bool isAddUseLog = true)
        {
			try {
				//保存
				model.Save();
				...
				if (isAddUseLog)
				{
					...
					{
						content = "{0}" + (model.Id == 0 ? "添加" : "編輯") + "Departs記錄成功，ID為【" + model.Id + "】";
					}
				...
				var result = "執行DepartsBll.Save()函數出錯！";
		...
		{
			//設瞞Sql語句
			var sql = string.Format("delete from {0} where {1} = {2}", DepartsTable.TableName,  DepartsTable.Id, id);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了Departs表id為【" + id + "】的記錄！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//設瞞Sql語句
			var sql = string.Format("delete from {0} where {1} in ({2})", DepartsTable.TableName,  DepartsTable.Id, str);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了Departs表id為【" + str + "】的記錄！");
			}
		}

		/// <summary>
        /// 刪除Departs表記錄——?果使用了緩存，刪除成功後會?空本表的所有緩存記錄，?後重新加載進緩存
        /// </summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="expression">條件語句</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
        public void Delete(Page page, Expression<Func<Departs, bool>> expression, bool isAddUseLog = true)
        {
			//執行刪除
			Departs.Delete(expression);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了Departs表記錄！");
			}
        }
        ...
        {
            //設瞞Sql語句
            var sql = string.Format("delete from {0}", DepartsTable.TableName);
            ...
            {
                //添加用戶操作記錄
                UseLogBll.GetInstence().Save(page, "{0}刪除了Departs表所有記錄！");
            }
        }
		#endregion
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "Departs", sortName, "depart_id"))
		    {
				//判斷是否?用緩存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}更新了Departs表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("depart_id", "Departs", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判斷是否?用緩存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}對Departs表進行了自動排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
	        ...
	        else
	        {
				if (string.IsNullOrEmpty(conditionColName))
	            {
	                conditionColName = DepartsTable.Id;
	            }
        ...
        /// <param name="expression">條件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.Departs, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<Departs>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.Departs model, string colName)
		{
			...
			switch (colName)
			{
				...
				case "inside_id" :
					return model.inside_id;
				case "depart_name" :
					return model.depart_name;
				case "manager1" :
					return model.manager1;
				case "job_id1" :
					return model.job_id1;
				case "manager2" :
					return model.manager2;
				case "job_id2" :
					return model.job_id2;
				case "emp_prefix" :
					return model.emp_prefix;
				case "plan_num" :
					return model.plan_num;
				case "linktel" :
					return model.linktel;
				case "remark" :
					return model.remark;
				case "butei_money" :
					return model.butei_money;
				case "audit_access" :
					return model.audit_access;
				case "TreeLevel" :
					return model.TreeLevel;
				case "Up_ID" :
					return model.Up_ID;
				case "LongName" :
		...
		#region 更新Departs表指定字段值
		/// <summary>更新Departs表記錄指定字段值，?果使用了緩存，保存成功後會?空本表的所有緩存記錄，?後重新加載進緩存</summary>
		/// <param name="page">當?頁面指針</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionHelper.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<Departs>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用戶操作記錄
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}囊改了Departs表記錄。");				
				}
				else
				{
					//添加用戶操作記錄
		...
		#region 更新Departs表指定主鍵Id的字段值
		/// <summary>更新Departs表記錄指定字段值</summary>
        /// <param name="page">當?頁面指針</param>
        ...
        {
			content = content != "" ? content : "{0}囊改了Departs表主鍵Id值為" + id + "的記錄。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionHelper.SqlqueryCondition(ConstraintType.And, DepartsTable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新Departs表記錄指定字段值（更新一個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了Departs表主鍵Id值為" + id + "的記錄，將" + columnName + "字段值囊改為" + columnValue;
            //設瞞更新字段
		 ...
		 /// <summary>更新Departs表記錄指定字段值（更新兩個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了Departs表主鍵Id值為" + id + "的記錄，將" + columnName1 + "字段值囊改為" + columnValue1 + "，" + columnName2 + "字段值囊改為" + columnValue2;
            //設瞞更新字段
            var dic = new Dictionary<string, object>();
        ...
        /// <param name="isCache">是否從緩存中讀?</param>
        /// <returns></returns>
        public string Getdepart_name(Page page, int pkValue, bool isCache = true)
        {
            //判斷是否?有用緩存
            if (isCache && CommonBll.IsUseCache())
            {
                ...
                return model == null ? "" : model.depart_name;
            }
            else
            {
                //從數據庫中查詢
                var model = Departs.SingleOrDefault(x => x.Id == pkValue);
                return model == null ? "" : model.depart_name;
            }
        }
        #endregion

		#region 獲取LongName字段值
        /// <summary>
        /// 獲?LongName字段值
        /// </summary>
        /// <param name="page">當?頁面指針</param>
        ...
        /// <param name="isCache">是否從緩存中讀?</param>
        /// <returns></returns>
        public string GetLongName(Page page, int pkValue, bool isCache = true)
        {
            //判斷是否?有用緩存
            if (isCache && CommonBll.IsUseCache())
            {
                //從緩存中獲取實伐
                var model = GetModelForCache(pkValue);
                return model == null ? "" : model.LongName;
            }
            else
            {
                //從數據庫中查詢
