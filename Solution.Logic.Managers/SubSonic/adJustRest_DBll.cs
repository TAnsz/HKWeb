...
namespace Solution.Logic.Managers {
	...
	public partial class adJustRest_DBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_adJustRest_D";
        private const string const_CacheKey_Date = "Cache_adJustRest_D_Date";

		#region 單例模式
		//定義單例實伐
		private static adJustRest_DBll _adJustRest_DBll = null;

		/// <summary>
		/// 獲取本邏輯類單例
		/// </summary>
		/// <returns></returns>
		public static adJustRest_DBll GetInstence() {
			if (_adJustRest_DBll == null) {
				_adJustRest_DBll = new adJustRest_DBll();
			}
			return _adJustRest_DBll;
		}
		#endregion
		...
		#region 從IIS緩存中獲取adJustRest_D表記錄
		/// <summary>
        /// 從IIS緩存中獲取adJustRest_D表記錄
        /// </summary>
	    /// <param name="isCache">是否從緩存中讀取</param>
        public IList<DataAccess.Model.adJustRest_D> GetList(bool isCache = true)
        {
			try
			{
				//判斷是否使用緩存
				if (CommonBll.IsUseCache() && isCache){
					...
					{
						return (IList<DataAccess.Model.adJustRest_D>)obj;
					}
				}
				else
				{
					//定義臨時實伐集
					IList<DataAccess.Model.adJustRest_D> list = null;

					//獲??表緩存加載條件表達式
					var exp = GetExpression<adJustRest_D>();
                    //?果條件為空，則查詢?表所有記錄
					if (exp == null)
					{
						//從數據庫中獲?所有記錄
						var all = adJustRest_D.All();
                        list = all == null ? null : Transform(all.ToList());
					}
					else
					{
                        //從數據庫中查詢出指定條件的記錄，並轉換為指定實伐集
						var all = adJustRest_D.Find(exp);
            ...
            {
                //記錄?志
                CommonBll.WriteLog("從IIS緩存中獲取adJustRest_D表記錄時出現異常", e);
			}
		...
		/// <returns>DataAccess.Model.adJustRest_D</returns>
        public DataAccess.Model.adJustRest_D GetModel(long id, bool isCache = true)
        {
            //判斷是否使用緩存
		    if (CommonBll.IsUseCache() && isCache)
		    {
		    ...
		    {
                //從數據庫中直接讀?
                var model = adJustRest_D.SingleOrDefault(x => x.Id == id);
                if (model == null)
                {
                    return null;
                }
        ...
        /// <param name="id">主鍵Id</param>
		/// <returns>DataAccess.Model.adJustRest_D</returns>
        public DataAccess.Model.adJustRest_D GetModelForCache(long id)
        {
			try
			{
				//從緩存中讀?指定Id記錄
				...
				if (model == null){
					//從數據庫中讀?
					var tem = adJustRest_D.SingleOrDefault(x => x.Id == id);
					if (tem == null)
					{
						return null;
					}
        ...
        /// <returns>DataAccess.Model.adJustRest_D</returns>
        public DataAccess.Model.adJustRest_D GetModelForCache(string conditionColName, object value)
        {
			try
            {
                //從緩存中獲取List
                var list = GetList();
                DataAccess.Model.adJustRest_D model = null;
                Expression<Func<adJustRest_D, bool>> expression = null;
                ...
                switch (conditionColName)
                {
					...
					case "depart_id" :
                        ...
                        break;
					case "ori_date" :
						model = list.SingleOrDefault(x => x.ori_date == (DateTime)value);
                        expression = x => x.ori_date == (DateTime)value;
                        break;
					case "ori_btime" :
						model = list.SingleOrDefault(x => x.ori_btime == (DateTime)value);
                        expression = x => x.ori_btime == (DateTime)value;
                        break;
					case "ori_etime" :
						model = list.SingleOrDefault(x => x.ori_etime == (DateTime)value);
                        expression = x => x.ori_etime == (DateTime)value;
                        break;
					case "rest_date" :
						model = list.SingleOrDefault(x => x.rest_date == (DateTime)value);
                        expression = x => x.rest_date == (DateTime)value;
                        break;
					case "rest_btime" :
						model = list.SingleOrDefault(x => x.rest_btime == (DateTime)value);
                        expression = x => x.rest_btime == (DateTime)value;
                        break;
					case "rest_etime" :
						model = list.SingleOrDefault(x => x.rest_etime == (DateTime)value);
                        expression = x => x.rest_etime == (DateTime)value;
                        break;
					case "checker" :
					...
					case "memo" :
                        ...
                        break;
					case "all_day" :
						model = list.SingleOrDefault(x => x.all_day == (short)value);
                        expression = x => x.all_day == (short)value;
                        break;
					case "kind" :
						model = list.SingleOrDefault(x => x.kind == (string)value);
                        expression = x => x.kind == (string)value;
                        break;
					case "refuse_reason" :
                ...
                {
                    //從數據庫中讀取
                    var tem = adJustRest_D.SingleOrDefault(expression);
                    if (tem == null)
                    {
                        return null;
                    }
        ...
        /// <returns>DataAccess.Model.adJustRest_D</returns>
        public DataAccess.Model.adJustRest_D GetModelForCache(Expression<Func<DataAccess.Model.adJustRest_D, bool>> expression)
        {
			//從緩存中讀?記錄列表
			var list = GetList();
            //?果條件為空，則查詢?表所有記錄
        ...
        public void SetModelForCache(DataAccess.Model.adJustRest_D model)
        {
		    ...
		    if (list == null)
		    {
                list = new List<DataAccess.Model.adJustRest_D>();
		    }
            //添加記錄
            list.Add(model);
        }
        ...
        /// </summary>
        /// <param name="model">記錄實伐</param>
        public void SetModelForCache(adJustRest_D model)
        {
		...
		/// <summary>
        /// 按條件刪除IIS緩存中adJustRest_D表的指定記錄
        /// </summary>
        /// <param name="expression">條件，值為null時刪除?有記錄</param>
		public void DelCache(Expression<Func<DataAccess.Model.adJustRest_D, bool>> expression)
        {
            //從緩存中獲?List
		    var list = GetList();
            //?果緩存為null，則不做?何處理
		...
		#region 實伐轉換
		/// <summary>
		/// 將adJustRest_D記錄實伐（SubSonic實伐）轉換為?通的實伐（DataAccess.Model.adJustRest_D）
		/// </summary>
        /// <param name="model">SubSonic插件生成的實伐</param>
		/// <returns>DataAccess.Model.adJustRest_D</returns>
		public DataAccess.Model.adJustRest_D Transform(adJustRest_D model)
        {			
			if (model == null) 
				return null;

            return new DataAccess.Model.adJustRest_D
            {
                Id = model.Id,
                ...
                ori_date = model.ori_date,
                ori_btime = model.ori_btime,
                ori_etime = model.ori_etime,
                rest_date = model.rest_date,
                rest_btime = model.rest_btime,
                rest_etime = model.rest_etime,
		...
		/// <summary>
		/// 將adJustRest_D記錄實伐集（SubSonic實伐）轉換為?通的實伐集（DataAccess.Model.adJustRest_D）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的實伐集</param>
        public IList<DataAccess.Model.adJustRest_D> Transform(IList<adJustRest_D> sourceList)
        {
			//創建List??
            var list = new List<DataAccess.Model.adJustRest_D>();
		...
		/// <summary>
		/// 將adJustRest_D記錄實伐批?通的實伐（DataAccess.Model.adJustRest_D）轉換為SubSonic插件生成的實伐
		/// </summary>
        /// <param name="model">?通的實伐（DataAccess.Model.adJustRest_D）</param>
		/// <returns>adJustRest_D</returns>
		public adJustRest_D Transform(DataAccess.Model.adJustRest_D model)
        {
			if (model == null) 
				return null;

            return new adJustRest_D
            {
                Id = model.Id,
                bill_id = model.bill_id,
                emp_id = model.emp_id,
		...
		/// <summary>
		/// 將adJustRest_D記錄實伐批?通實伐集（DataAccess.Model.adJustRest_D）轉換為SubSonic插件生成的實伐集
		/// </summary>
        /// <param name="sourceList">?通實伐集（DataAccess.Model.adJustRest_D）</param>
        public IList<adJustRest_D> Transform(IList<DataAccess.Model.adJustRest_D> sourceList)
        {
			//創建List??
            var list = new List<adJustRest_D>();
		...
		public void SetModelValue(DataAccess.Model.adJustRest_D model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.adJustRest_D model, string colName, object value)
		{
            ...
            switch (colName)
            {
				...
				case "depart_id" :
                    ...
                    break;
				case "ori_date" :
					model.ori_date = (DateTime)value;
                    break;
				case "ori_btime" :
					model.ori_btime = (DateTime)value;
                    break;
				case "ori_etime" :
					model.ori_etime = (DateTime)value;
                    break;
				case "rest_date" :
					model.rest_date = (DateTime)value;
                    break;
				case "rest_btime" :
					model.rest_btime = (DateTime)value;
                    break;
				case "rest_etime" :
					model.rest_etime = (DateTime)value;
                    break;
				case "checker" :
				...
				case "memo" :
                    ...
                    break;
				case "all_day" :
					model.all_day = (short)value;
                    break;
				case "kind" :
					model.kind = (string)value;
                    break;
				case "refuse_reason" :
        ...
        public int GetRecordCount()
        {
			...
			else
			{
				...
				return select.GetRecordCount<adJustRest_D>();
			}
        }

		/// <summary>
		/// 獲?adJustRest_D表記錄總數——從數據庫中查詢
		/// </summary>
        /// <param name="wheres">條件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionHelper.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<adJustRest_D>(wheres);

		}

		/// <summary>
		/// 獲?adJustRest_D表指定條件的記錄總數——從數據庫中查詢
		/// </summary>
        /// <param name="expression">條件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<adJustRest_D, bool>> expression) {
            return new Select().From<adJustRest_D>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.adJustRest_D> Find(Expression<Func<DataAccess.Model.adJustRest_D, bool>> expression)
        {
			//從緩存中獲?記錄集
			var list = GetList();
            //判斷獲?記錄集是否為null
        ...
        public bool Exist(int id)
        {
            ...
            //從數據庫中查找
            return adJustRest_D.Exists(x => x.Id == id);
        }
        ...
        /// <param name="expression">條件語句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.adJustRest_D, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionHelper.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分頁查詢
                var select = new SelectHelper();
                return select.SelectDataTable<adJustRest_D>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //記錄?志
                CommonBll.WriteLog("獲?adJustRest_D表記錄時出現異常", e);
		...
		#region 添加與編輯adJustRest_D表記錄
		/// <summary>
		/// 添加與編輯adJustRest_D記錄
		/// </summary>
	    /// <param name="page">當?頁面指針</param>
		/// <param name="model">adJustRest_D表實伐</param>
        /// <param name="content">更新說明</param>
        /// <param name="isCache">是否更新緩存</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
        public void Save(Page page, adJustRest_D model, string content = null, bool isCache = true, bool isAddUseLog = true)
        {
			try {
				//保存
				model.Save();
				...
				if (isAddUseLog)
				{
					...
					{
						content = "{0}" + (model.Id == 0 ? "添加" : "編輯") + "adJustRest_D記錄成功，ID為【" + model.Id + "】";
					}
				...
				var result = "執行adJustRest_DBll.Save()函數出錯！";
		...
		{
			//設瞞Sql語句
			var sql = string.Format("delete from {0} where {1} = {2}", adJustRest_DTable.TableName,  adJustRest_DTable.Id, id);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了adJustRest_D表id為【" + id + "】的記錄！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//設瞞Sql語句
			var sql = string.Format("delete from {0} where {1} in ({2})", adJustRest_DTable.TableName,  adJustRest_DTable.Id, str);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了adJustRest_D表id為【" + str + "】的記錄！");
			}
		}

		/// <summary>
        /// 刪除adJustRest_D表記錄——?果使用了緩存，刪除成功後會?空本表的所有緩存記錄，?後重新加載進緩存
        /// </summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="expression">條件語句</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
        public void Delete(Page page, Expression<Func<adJustRest_D, bool>> expression, bool isAddUseLog = true)
        {
			//執行刪除
			adJustRest_D.Delete(expression);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了adJustRest_D表記錄！");
			}
        }
        ...
        {
            //設瞞Sql語句
            var sql = string.Format("delete from {0}", adJustRest_DTable.TableName);
            ...
            {
                //添加用戶操作記錄
                UseLogBll.GetInstence().Save(page, "{0}刪除了adJustRest_D表所有記錄！");
            }
        }
		#endregion
		...
		public override bool UpdateSort(Page page, FineUI.Grid grid1, string tbxSort, string sortName = "Sort")
	    {
			...
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "adJustRest_D", sortName, "Id"))
		    {
			    ...
			    //添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}更新了adJustRest_D表排序！");
		...
		public override bool UpdateAutoSort(Page page, string strWhere = "", bool isExistsMoreLv = false, int pid = 0, string fieldName = "Sort", string fieldParentId = "ParentId")
	    {
			...
			if (CommonBll.AutoSort("Id", "adJustRest_D", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
			    ...
			    //添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}對adJustRest_D表進行了自動排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
	        ...
	        else
	        {
				if (string.IsNullOrEmpty(conditionColName))
	            {
	                conditionColName = adJustRest_DTable.Id;
	            }
        ...
        /// <param name="expression">條件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.adJustRest_D, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<adJustRest_D>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.adJustRest_D model, string colName)
		{
			...
			switch (colName)
			{
				...
				case "ori_date" :
					return model.ori_date;
				case "ori_btime" :
					return model.ori_btime;
				case "ori_etime" :
					return model.ori_etime;
				case "rest_date" :
					return model.rest_date;
				case "rest_btime" :
					return model.rest_btime;
				case "rest_etime" :
					return model.rest_etime;
				case "checker" :
		...
		#region 更新adJustRest_D表指定字段值
		/// <summary>更新adJustRest_D表記錄指定字段值，?果使用了緩存，保存成功後會?空本表的所有緩存記錄，?後重新加載進緩存</summary>
		/// <param name="page">當?頁面指針</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionHelper.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<adJustRest_D>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用戶操作記錄
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}囊改了adJustRest_D表記錄。");				
				}
				else
				{
					//添加用戶操作記錄
		...
		#region 更新adJustRest_D表指定主鍵Id的字段值
		/// <summary>更新adJustRest_D表記錄指定字段值</summary>
        /// <param name="page">當?頁面指針</param>
        ...
        {
			content = content != "" ? content : "{0}囊改了adJustRest_D表主鍵Id值為" + id + "的記錄。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionHelper.SqlqueryCondition(ConstraintType.And, adJustRest_DTable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新adJustRest_D表記錄指定字段值（更新一個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了adJustRest_D表主鍵Id值為" + id + "的記錄，將" + columnName + "字段值囊改為" + columnValue;
            //設瞞更新字段
		 ...
		 /// <summary>更新adJustRest_D表記錄指定字段值（更新兩個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了adJustRest_D表主鍵Id值為" + id + "的記錄，將" + columnName1 + "字段值囊改為" + columnValue1 + "，" + columnName2 + "字段值囊改為" + columnValue2;
            //設瞞更新字段
            var dic = new Dictionary<string, object>();
