...
namespace Solution.Logic.Managers {
	...
	public partial class MenuInfoBll : LogicBase {
		...
		#region 從IIS緩存中獲取MenuInfo表記錄
		/// <summary>
        /// 從IIS緩存中獲取MenuInfo表記錄
        /// </summary>
        ...
        public IList<DataAccess.Model.MenuInfo> GetList(bool isCache = true)
        {
            ...
            {
                //記錄?志
                CommonBll.WriteLog("從IIS緩存中獲取MenuInfo表記錄時出現異常", e);
			}
		...
		#region 實伐轉換
		/// <summary>
		/// 將MenuInfo記錄實伐（SubSonic實伐）轉換為?通的實伐（DataAccess.Model.MenuInfo）
		/// </summary>
		...
		/// <summary>
		/// 將MenuInfo記錄實伐集（SubSonic實伐）轉換為?通的實伐集（DataAccess.Model.MenuInfo）
		/// </summary>
		...
		/// <summary>
		/// 將MenuInfo記錄實伐批?通的實伐（DataAccess.Model.MenuInfo）轉換為SubSonic插件生成的實伐
		/// </summary>
        /// <param name="model">?通的實伐（DataAccess.Model.MenuInfo）</param>
		...
		/// <summary>
		/// 將MenuInfo記錄實伐批?通實伐集（DataAccess.Model.MenuInfo）轉換為SubSonic插件生成的實伐集
		/// </summary>
        /// <param name="sourceList">?通實伐集（DataAccess.Model.MenuInfo）</param>
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionHelper.SqlqueryCondition> wheres = null, List<string> sorts = null) {
            ...
            {
                //記錄?志
                CommonBll.WriteLog("獲?MenuInfo表記錄時出現異常", e);
		...
		#region 添加與編輯MenuInfo表記錄
		/// <summary>
		/// 添加與編輯MenuInfo記錄
		/// </summary>
	    /// <param name="page">當?頁面指針</param>
		/// <param name="model">MenuInfo表實伐</param>
        ...
        public void Save(Page page, MenuInfo model, string content = null, bool isCache = true, bool isAddUseLog = true)
        {
			try {
				...
				if (isAddUseLog)
				{
					...
					{
						content = "{0}" + (model.Id == 0 ? "添加" : "編輯") + "MenuInfo記錄成功，ID為【" + model.Id + "】";
					}
				...
				var result = "執行MenuInfoBll.Save()函數出錯！";
		...
		public override void Delete(Page page, int id, bool isAddUseLog = true) 
		{
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了MenuInfo表id為【" + id + "】的記錄！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了MenuInfo表id為【" + str + "】的記錄！");
			}
		}

		/// <summary>
        /// 刪除MenuInfo表記錄——?果使用了緩存，刪除成功後會?空本表的所有緩存記錄，?後重新加載進緩存
        /// </summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="expression">條件語句</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
        public void Delete(Page page, Expression<Func<MenuInfo, bool>> expression, bool isAddUseLog = true)
        {
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了MenuInfo表記錄！");
			}
        }
        ...
        public void DeleteAll(Page page, bool isAddUseLog = true)
        {
            ...
            {
                //添加用戶操作記錄
                UseLogBll.GetInstence().Save(page, "{0}刪除了MenuInfo表所有記錄！");
            }
        }
		#endregion
		...
		public override bool UpdateAutoSort(Page page, string strWhere = "", bool isExistsMoreLv = false, int pid = 0, string fieldName = "Sort", string fieldParentId = "ParentId")
	    {
			...
			if (CommonBll.AutoSort("Id", "MenuInfo", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
			    ...
			    //添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}對MenuInfo表進行了自動排序操作！");
		...
		/// <summary>更新MenuInfo表記錄指定字段值，?果使用了緩存，保存成功後會?空本表的所有緩存記錄，?後重新加載進緩存</summary>
		/// <param name="page">當?頁面指針</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionHelper.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			if (isAddUseLog){
				...
				{
					//添加用戶操作記錄
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}囊改了MenuInfo表記錄。");				
				}
				else
				{
					//添加用戶操作記錄
		...
		#region 更新MenuInfo表指定主鍵Id的字段值
		/// <summary>更新MenuInfo表記錄指定字段值</summary>
        /// <param name="page">當?頁面指針</param>
        ...
        {
			content = content != "" ? content : "{0}囊改了MenuInfo表主鍵Id值為" + id + "的記錄。";
        ...
        /// <summary>更新MenuInfo表記錄指定字段值（更新一個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了MenuInfo表主鍵Id值為" + id + "的記錄，將" + columnName + "字段值囊改為" + columnValue;
            //設瞞更新字段
		 ...
		 /// <summary>更新MenuInfo表記錄指定字段值（更新兩個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了MenuInfo表主鍵Id值為" + id + "的記錄，將" + columnName1 + "字段值囊改為" + columnValue1 + "，" + columnName2 + "字段值囊改為" + columnValue2;
            //設瞞更新字段
            var dic = new Dictionary<string, object>();
		...
		public void UpdateIsDisplay(Page page, int pkValue, int updateValue, bool isCache = true, bool isAddUseLog = true) {
			...
			//更新
			UpdateValue(page, pkValue, setValue, "{0}更新了MenuInfo表id為【" + pkValue + "】的記錄，更新內容為將IsDisplay字段值囊改為" + updateValue, isCache, isAddUseLog);
		}
		#endregion
		...
		public void UpdateIsMenu(Page page, int pkValue, int updateValue, bool isCache = true, bool isAddUseLog = true) {
			...
			//更新
			UpdateValue(page, pkValue, setValue, "{0}更新了MenuInfo表id為【" + pkValue + "】的記錄，更新內容為將IsMenu字段值囊改為" + updateValue, isCache, isAddUseLog);
		}
		#endregion
