...
namespace Solution.Logic.Managers {
	...
	public partial class UploadTypeBll : LogicBase {
		...
		#region 從IIS緩存中獲取UploadType表記錄
		/// <summary>
        /// 從IIS緩存中獲取UploadType表記錄
        /// </summary>
        ...
        public IList<DataAccess.Model.UploadType> GetList(bool isCache = true)
        {
            ...
            {
                //記錄?志
                CommonBll.WriteLog("從IIS緩存中獲取UploadType表記錄時出現異常", e);
			}
		...
		#region 實伐轉換
		/// <summary>
		/// 將UploadType記錄實伐（SubSonic實伐）轉換為?通的實伐（DataAccess.Model.UploadType）
		/// </summary>
		...
		/// <summary>
		/// 將UploadType記錄實伐集（SubSonic實伐）轉換為?通的實伐集（DataAccess.Model.UploadType）
		/// </summary>
		...
		/// <summary>
		/// 將UploadType記錄實伐批?通的實伐（DataAccess.Model.UploadType）轉換為SubSonic插件生成的實伐
		/// </summary>
        /// <param name="model">?通的實伐（DataAccess.Model.UploadType）</param>
		...
		/// <summary>
		/// 將UploadType記錄實伐批?通實伐集（DataAccess.Model.UploadType）轉換為SubSonic插件生成的實伐集
		/// </summary>
        /// <param name="sourceList">?通實伐集（DataAccess.Model.UploadType）</param>
		...
		/// <summary>
		/// 獲?UploadType表指定條件的記錄總數——從數據庫中查詢
		/// </summary>
        /// <param name="expression">條件</param>
		/// <returns>int</returns>
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionHelper.SqlqueryCondition> wheres = null, List<string> sorts = null) {
            ...
            {
                //記錄?志
                CommonBll.WriteLog("獲?UploadType表記錄時出現異常", e);
		...
		#region 添加與編輯UploadType表記錄
		/// <summary>
		/// 添加與編輯UploadType記錄
		/// </summary>
	    /// <param name="page">當?頁面指針</param>
		/// <param name="model">UploadType表實伐</param>
        ...
        public void Save(Page page, UploadType model, string content = null, bool isCache = true, bool isAddUseLog = true)
        {
			try {
				...
				if (isAddUseLog)
				{
					...
					{
						content = "{0}" + (model.Id == 0 ? "添加" : "編輯") + "UploadType記錄成功，ID為【" + model.Id + "】";
					}
				...
				var result = "執行UploadTypeBll.Save()函數出錯！";
		...
		public override void Delete(Page page, int id, bool isAddUseLog = true) 
		{
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了UploadType表id為【" + id + "】的記錄！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了UploadType表id為【" + str + "】的記錄！");
			}
		}

		/// <summary>
        /// 刪除UploadType表記錄——?果使用了緩存，刪除成功後會?空本表的所有緩存記錄，?後重新加載進緩存
        /// </summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="expression">條件語句</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
        public void Delete(Page page, Expression<Func<UploadType, bool>> expression, bool isAddUseLog = true)
        {
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了UploadType表記錄！");
			}
        }
        ...
        public void DeleteAll(Page page, bool isAddUseLog = true)
        {
            ...
            {
                //添加用戶操作記錄
                UseLogBll.GetInstence().Save(page, "{0}刪除了UploadType表所有記錄！");
            }
        }
		#endregion
		...
		public override bool UpdateAutoSort(Page page, string strWhere = "", bool isExistsMoreLv = false, int pid = 0, string fieldName = "Sort", string fieldParentId = "ParentId")
	    {
			...
			if (CommonBll.AutoSort("Id", "UploadType", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
			    ...
			    //添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}對UploadType表進行了自動排序操作！");
		...
		/// <summary>更新UploadType表記錄指定字段值，?果使用了緩存，保存成功後會?空本表的所有緩存記錄，?後重新加載進緩存</summary>
		/// <param name="page">當?頁面指針</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionHelper.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			if (isAddUseLog){
				...
				{
					//添加用戶操作記錄
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}囊改了UploadType表記錄。");				
				}
				else
				{
					//添加用戶操作記錄
		...
		#region 更新UploadType表指定主鍵Id的字段值
		/// <summary>更新UploadType表記錄指定字段值</summary>
        /// <param name="page">當?頁面指針</param>
        ...
        {
			content = content != "" ? content : "{0}囊改了UploadType表主鍵Id值為" + id + "的記錄。";
        ...
        /// <summary>更新UploadType表記錄指定字段值（更新一個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了UploadType表主鍵Id值為" + id + "的記錄，將" + columnName + "字段值囊改為" + columnValue;
            //設瞞更新字段
		 ...
		 /// <summary>更新UploadType表記錄指定字段值（更新兩個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了UploadType表主鍵Id值為" + id + "的記錄，將" + columnName1 + "字段值囊改為" + columnValue1 + "，" + columnName2 + "字段值囊改為" + columnValue2;
            //設瞞更新字段
            var dic = new Dictionary<string, object>();
		...
		#region 使用TypeKey來查詢，獲?一個UploadType實伐對像
        /// <summary>使用Key來查詢，獲取一個UploadType實伐對像</summary>
        /// <param name="page">當前頁面指針</param>
        /// <param name="key">Key值</param>
        /// <param name="isCache">是否從緩存中讀取</param>
        /// <returns>DataAccess.Model.UploadType 實伐</returns>
		...
		public void UpdateIsSys(Page page, int pkValue, int updateValue, bool isCache = true, bool isAddUseLog = true) {
			...
			//更新
			UpdateValue(page, pkValue, setValue, "{0}更新了UploadType表id為【" + pkValue + "】的記錄，更新內容為將IsSys字段值囊改為" + updateValue, isCache, isAddUseLog);
		}
		#endregion
