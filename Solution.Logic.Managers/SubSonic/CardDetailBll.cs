...
namespace Solution.Logic.Managers {
	...
	/// CardDetail表邏輯類
	/// </summary>
	public partial class CardDetailBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_CardDetail";
        private const string const_CacheKey_Date = "Cache_CardDetail_Date";

		#region 單例模式
		//定義單例實伐
		private static CardDetailBll _CardDetailBll = null;

		/// <summary>
		/// 獲取本邏輯類單例
		/// </summary>
		/// <returns></returns>
		public static CardDetailBll GetInstence() {
			if (_CardDetailBll == null) {
				_CardDetailBll = new CardDetailBll();
			}
			return _CardDetailBll;
		}
		#endregion
		...
		#region 從IIS緩存中獲取CardDetail表記錄
		/// <summary>
        /// 從IIS緩存中獲取CardDetail表記錄
        /// </summary>
	    /// <param name="isCache">是否從緩存中讀取</param>
        public IList<DataAccess.Model.CardDetail> GetList(bool isCache = true)
        {
			try
			{
				//判斷是否使用緩存
				if (CommonBll.IsUseCache() && isCache){
					...
					{
						return (IList<DataAccess.Model.CardDetail>)obj;
					}
				}
				else
				{
					//定義臨時實伐集
					IList<DataAccess.Model.CardDetail> list = null;

					//獲??表緩存加載條件表達式
					var exp = GetExpression<CardDetail>();
                    //?果條件為空，則查詢?表所有記錄
					if (exp == null)
					{
						//從數據庫中獲?所有記錄
						var all = CardDetail.All();
                        list = all == null ? null : Transform(all.ToList());
					}
					else
					{
                        //從數據庫中查詢出指定條件的記錄，並轉換為指定實伐集
						var all = CardDetail.Find(exp);
            ...
            {
                //記錄?志
                CommonBll.WriteLog("從IIS緩存中獲取CardDetail表記錄時出現異常", e);
			}
		...
		/// <returns>DataAccess.Model.CardDetail</returns>
        public DataAccess.Model.CardDetail GetModel(long id, bool isCache = true)
        {
            //判斷是否使用緩存
		    if (CommonBll.IsUseCache() && isCache)
		    {
		    ...
		    {
                //從數據庫中直接讀?
                var model = CardDetail.SingleOrDefault(x => x.Id == id);
                if (model == null)
                {
                    return null;
                }
        ...
        /// <param name="id">主鍵Id</param>
		/// <returns>DataAccess.Model.CardDetail</returns>
        public DataAccess.Model.CardDetail GetModelForCache(long id)
        {
			try
			{
				//從緩存中讀?指定Id記錄
				...
				if (model == null){
					//從數據庫中讀?
					var tem = CardDetail.SingleOrDefault(x => x.Id == id);
					if (tem == null)
					{
						return null;
					}
        ...
        /// <returns>DataAccess.Model.CardDetail</returns>
        public DataAccess.Model.CardDetail GetModelForCache(string conditionColName, object value)
        {
			try
            {
                //從緩存中獲取List
                var list = GetList();
                DataAccess.Model.CardDetail model = null;
                Expression<Func<CardDetail, bool>> expression = null;
                ...
                switch (conditionColName)
                {
					...
					case "kind" :
                        ...
                        break;
					case "use_date" :
						model = list.SingleOrDefault(x => x.use_date == (DateTime)value);
                        expression = x => x.use_date == (DateTime)value;
                        break;
					case "Invalid_date" :
						model = list.SingleOrDefault(x => x.Invalid_date == (DateTime)value);
                        expression = x => x.Invalid_date == (DateTime)value;
                        break;
					case "isbuban" :
						model = list.SingleOrDefault(x => x.isbuban == (short)value);
                        expression = x => x.isbuban == (short)value;
                        break;
					case "isunloss" :
						model = list.SingleOrDefault(x => x.isunloss == (short)value);
                        expression = x => x.isunloss == (short)value;
                        break;
					case "card_sn" :
                        ...
                        break;
					case "old_card_id" :
						model = list.SingleOrDefault(x => x.old_card_id == (string)value);
                        expression = x => x.old_card_id == (string)value;
                        break;
					case "audit" :
                ...
                {
                    //從數據庫中讀取
                    var tem = CardDetail.SingleOrDefault(expression);
                    if (tem == null)
                    {
                        return null;
                    }
        ...
        /// <returns>DataAccess.Model.CardDetail</returns>
        public DataAccess.Model.CardDetail GetModelForCache(Expression<Func<DataAccess.Model.CardDetail, bool>> expression)
        {
			//從緩存中讀?記錄列表
			var list = GetList();
            //?果條件為空，則查詢?表所有記錄
        ...
        public void SetModelForCache(DataAccess.Model.CardDetail model)
        {
		    ...
		    if (list == null)
		    {
                list = new List<DataAccess.Model.CardDetail>();
		    }
            //添加記錄
            list.Add(model);
        }
        ...
        /// </summary>
        /// <param name="model">記錄實伐</param>
        public void SetModelForCache(CardDetail model)
        {
		...
		/// <summary>
        /// 按條件刪除IIS緩存中CardDetail表的指定記錄
        /// </summary>
        /// <param name="expression">條件，值為null時刪除?有記錄</param>
		public void DelCache(Expression<Func<DataAccess.Model.CardDetail, bool>> expression)
        {
            //從緩存中獲?List
		    var list = GetList();
            //?果緩存為null，則不做?何處理
		...
		#region 實伐轉換
		/// <summary>
		/// 將CardDetail記錄實伐（SubSonic實伐）轉換為?通的實伐（DataAccess.Model.CardDetail）
		/// </summary>
        /// <param name="model">SubSonic插件生成的實伐</param>
		/// <returns>DataAccess.Model.CardDetail</returns>
		public DataAccess.Model.CardDetail Transform(CardDetail model)
        {			
			if (model == null) 
				return null;

            return new DataAccess.Model.CardDetail
            {
                Id = model.Id,
                emp_id = model.emp_id,
                ...
                isbuban = model.isbuban,
                isunloss = model.isunloss,
                card_sn = model.card_sn,
                old_card_id = model.old_card_id,
                audit = model.audit,
                remark = model.remark,
		...
		/// <summary>
		/// 將CardDetail記錄實伐集（SubSonic實伐）轉換為?通的實伐集（DataAccess.Model.CardDetail）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的實伐集</param>
        public IList<DataAccess.Model.CardDetail> Transform(IList<CardDetail> sourceList)
        {
			//創建List??
            var list = new List<DataAccess.Model.CardDetail>();
		...
		/// <summary>
		/// 將CardDetail記錄實伐批?通的實伐（DataAccess.Model.CardDetail）轉換為SubSonic插件生成的實伐
		/// </summary>
        /// <param name="model">?通的實伐（DataAccess.Model.CardDetail）</param>
		/// <returns>CardDetail</returns>
		public CardDetail Transform(DataAccess.Model.CardDetail model)
        {
			if (model == null) 
				return null;
		...
		/// <summary>
		/// 將CardDetail記錄實伐批?通實伐集（DataAccess.Model.CardDetail）轉換為SubSonic插件生成的實伐集
		/// </summary>
        /// <param name="sourceList">?通實伐集（DataAccess.Model.CardDetail）</param>
        public IList<CardDetail> Transform(IList<DataAccess.Model.CardDetail> sourceList)
        {
			//創建List??
		...
		public void SetModelValue(DataAccess.Model.CardDetail model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.CardDetail model, string colName, object value)
		{
            ...
            switch (colName)
            {
				...
				case "kind" :
                    ...
                    break;
				case "use_date" :
					model.use_date = (DateTime)value;
                    break;
				case "Invalid_date" :
					model.Invalid_date = (DateTime)value;
                    break;
				case "isbuban" :
					model.isbuban = (short)value;
                    break;
				case "isunloss" :
					model.isunloss = (short)value;
                    break;
				case "card_sn" :
					model.card_sn = (string)value;
                    break;
				case "old_card_id" :
					model.old_card_id = (string)value;
                    break;
				case "audit" :
        ...
        public int GetRecordCount()
        {
			...
			else
			{
				...
				return select.GetRecordCount<CardDetail>();
			}
        }

		/// <summary>
		/// 獲?CardDetail表記錄總數——從數據庫中查詢
		/// </summary>
        /// <param name="wheres">條件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionHelper.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<CardDetail>(wheres);

		}

		/// <summary>
		/// 獲?CardDetail表指定條件的記錄總數——從數據庫中查詢
		/// </summary>
        /// <param name="expression">條件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<CardDetail, bool>> expression) {
            return new Select().From<CardDetail>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.CardDetail> Find(Expression<Func<DataAccess.Model.CardDetail, bool>> expression)
        {
			//從緩存中獲?記錄集
			var list = GetList();
            //判斷獲?記錄集是否為null
        ...
        public bool Exist(int id)
        {
            ...
            //從數據庫中查找
            return CardDetail.Exists(x => x.Id == id);
        }
        ...
        /// <param name="expression">條件語句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.CardDetail, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionHelper.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分頁查詢
                var select = new SelectHelper();
                return select.SelectDataTable<CardDetail>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //記錄?志
                CommonBll.WriteLog("獲?CardDetail表記錄時出現異常", e);
		...
		#region 添加與編輯CardDetail表記錄
		/// <summary>
		/// 添加與編輯CardDetail記錄
		/// </summary>
	    /// <param name="page">當?頁面指針</param>
		/// <param name="model">CardDetail表實伐</param>
        /// <param name="content">更新說明</param>
        /// <param name="isCache">是否更新緩存</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
        public void Save(Page page, CardDetail model, string content = null, bool isCache = true, bool isAddUseLog = true)
        {
			try {
				//保存
				model.Save();
				...
				if (isAddUseLog)
				{
					...
					{
						content = "{0}" + (model.Id == 0 ? "添加" : "編輯") + "CardDetail記錄成功，ID為【" + model.Id + "】";
					}
				...
				var result = "執行CardDetailBll.Save()函數出錯！";
		...
		{
			//設瞞Sql語句
			var sql = string.Format("delete from {0} where {1} = {2}", CardDetailTable.TableName,  CardDetailTable.Id, id);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了CardDetail表id為【" + id + "】的記錄！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//設瞞Sql語句
			var sql = string.Format("delete from {0} where {1} in ({2})", CardDetailTable.TableName,  CardDetailTable.Id, str);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了CardDetail表id為【" + str + "】的記錄！");
			}
		}

		/// <summary>
        /// 刪除CardDetail表記錄——?果使用了緩存，刪除成功後會?空本表的所有緩存記錄，?後重新加載進緩存
        /// </summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="expression">條件語句</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
        public void Delete(Page page, Expression<Func<CardDetail, bool>> expression, bool isAddUseLog = true)
        {
			//執行刪除
			CardDetail.Delete(expression);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了CardDetail表記錄！");
			}
        }
        ...
        {
            //設瞞Sql語句
            var sql = string.Format("delete from {0}", CardDetailTable.TableName);
            ...
            {
                //添加用戶操作記錄
                UseLogBll.GetInstence().Save(page, "{0}刪除了CardDetail表所有記錄！");
            }
        }
		#endregion
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "CardDetail", sortName, "Id"))
		    {
				//判斷是否?用緩存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}更新了CardDetail表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "CardDetail", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判斷是否?用緩存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}對CardDetail表進行了自動排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
	        ...
	        else
	        {
				if (string.IsNullOrEmpty(conditionColName))
	            {
	                conditionColName = CardDetailTable.Id;
	            }
        ...
        /// <param name="expression">條件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.CardDetail, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<CardDetail>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.CardDetail model, string colName)
		{
			...
			switch (colName)
			{
				...
				case "kind" :
					return model.kind;
				case "use_date" :
					return model.use_date;
				case "Invalid_date" :
					return model.Invalid_date;
				case "isbuban" :
					return model.isbuban;
				case "isunloss" :
					return model.isunloss;
				case "card_sn" :
					return model.card_sn;
				case "old_card_id" :
					return model.old_card_id;
				case "audit" :
					return model.audit;
				case "remark" :
					return model.remark;
		...
		#region 更新CardDetail表指定字段值
		/// <summary>更新CardDetail表記錄指定字段值，?果使用了緩存，保存成功後會?空本表的所有緩存記錄，?後重新加載進緩存</summary>
		/// <param name="page">當?頁面指針</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionHelper.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<CardDetail>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用戶操作記錄
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}囊改了CardDetail表記錄。");				
				}
				else
				{
					//添加用戶操作記錄
		...
		#region 更新CardDetail表指定主鍵Id的字段值
		/// <summary>更新CardDetail表記錄指定字段值</summary>
        /// <param name="page">當?頁面指針</param>
        ...
        {
			content = content != "" ? content : "{0}囊改了CardDetail表主鍵Id值為" + id + "的記錄。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionHelper.SqlqueryCondition(ConstraintType.And, CardDetailTable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新CardDetail表記錄指定字段值（更新一個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了CardDetail表主鍵Id值為" + id + "的記錄，將" + columnName + "字段值囊改為" + columnValue;
            //設瞞更新字段
		 ...
		 /// <summary>更新CardDetail表記錄指定字段值（更新兩個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了CardDetail表主鍵Id值為" + id + "的記錄，將" + columnName1 + "字段值囊改為" + columnValue1 + "，" + columnName2 + "字段值囊改為" + columnValue2;
            //設瞞更新字段
            var dic = new Dictionary<string, object>();
