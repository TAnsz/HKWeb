...
namespace Solution.Logic.Managers {
	...
	/// ActiveFile表邏輯類
	/// </summary>
	public partial class ActiveFileBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_ActiveFile";
        private const string const_CacheKey_Date = "Cache_ActiveFile_Date";

		#region 單例模式
		//定義單例實伐
		private static ActiveFileBll _ActiveFileBll = null;

		/// <summary>
		/// 獲取本邏輯類單例
		/// </summary>
		/// <returns></returns>
		public static ActiveFileBll GetInstence() {
			if (_ActiveFileBll == null) {
				_ActiveFileBll = new ActiveFileBll();
			}
			return _ActiveFileBll;
		}
		#endregion
		...
		#region 從IIS緩存中獲取ActiveFile表記錄
		/// <summary>
        /// 從IIS緩存中獲取ActiveFile表記錄
        /// </summary>
	    /// <param name="isCache">是否從緩存中讀取</param>
        public IList<DataAccess.Model.ActiveFile> GetList(bool isCache = true)
        {
			try
			{
				//判斷是否使用緩存
				if (CommonBll.IsUseCache() && isCache){
					...
					{
						return (IList<DataAccess.Model.ActiveFile>)obj;
					}
				}
				else
				{
					//定義臨時實伐集
					IList<DataAccess.Model.ActiveFile> list = null;

					//獲??表緩存加載條件表達式
					var exp = GetExpression<ActiveFile>();
                    //?果條件為空，則查詢?表所有記錄
					if (exp == null)
					{
						//從數據庫中獲?所有記錄
						var all = ActiveFile.All();
                        list = all == null ? null : Transform(all.ToList());
					}
					else
					{
                        //從數據庫中查詢出指定條件的記錄，並轉換為指定實伐集
						var all = ActiveFile.Find(exp);
            ...
            {
                //記錄?志
                CommonBll.WriteLog("從IIS緩存中獲取ActiveFile表記錄時出現異常", e);
			}
		...
		/// <returns>DataAccess.Model.ActiveFile</returns>
        public DataAccess.Model.ActiveFile GetModel(long id, bool isCache = true)
        {
            //判斷是否使用緩存
		    if (CommonBll.IsUseCache() && isCache)
		    {
		    ...
		    {
                //從數據庫中直接讀?
                var model = ActiveFile.SingleOrDefault(x => x.Id == id);
                if (model == null)
                {
                    return null;
                }
        ...
        /// <param name="id">主鍵Id</param>
		/// <returns>DataAccess.Model.ActiveFile</returns>
        public DataAccess.Model.ActiveFile GetModelForCache(long id)
        {
			try
			{
				//從緩存中讀?指定Id記錄
				...
				if (model == null){
					//從數據庫中讀?
					var tem = ActiveFile.SingleOrDefault(x => x.Id == id);
					if (tem == null)
					{
						return null;
					}
        ...
        /// <returns>DataAccess.Model.ActiveFile</returns>
        public DataAccess.Model.ActiveFile GetModelForCache(string conditionColName, object value)
        {
			try
            {
                //從緩存中獲取List
                var list = GetList();
                DataAccess.Model.ActiveFile model = null;
                Expression<Func<ActiveFile, bool>> expression = null;
                ...
                switch (conditionColName)
                {
					...
					case "Url" :
                        ...
                        break;
					case "DownloadCount" :
						model = list.SingleOrDefault(x => x.DownloadCount == (int)value);
                        expression = x => x.DownloadCount == (int)value;
                        break;
					case "Sort" :
					...
					case "UpdateDate" :
                        ...
                        break;
					case "ActiveFileClass_Id" :
						model = list.SingleOrDefault(x => x.ActiveFileClass_Id == (int)value);
                        expression = x => x.ActiveFileClass_Id == (int)value;
                        break;
					case "ActiveFileClass_Name" :
						model = list.SingleOrDefault(x => x.ActiveFileClass_Name == (string)value);
                        expression = x => x.ActiveFileClass_Name == (string)value;
                        break;
					case "Employee_EmpId" :
                        ...
                        break;
					case "Employee_CName" :
						model = list.SingleOrDefault(x => x.Employee_CName == (string)value);
                        expression = x => x.Employee_CName == (string)value;
                        break;
                ...
                {
                    //從數據庫中讀取
                    var tem = ActiveFile.SingleOrDefault(expression);
                    if (tem == null)
                    {
                        return null;
                    }
        ...
        /// <returns>DataAccess.Model.ActiveFile</returns>
        public DataAccess.Model.ActiveFile GetModelForCache(Expression<Func<DataAccess.Model.ActiveFile, bool>> expression)
        {
			//從緩存中讀?記錄列表
			var list = GetList();
            //?果條件為空，則查詢?表所有記錄
        ...
        public void SetModelForCache(DataAccess.Model.ActiveFile model)
        {
		    ...
		    if (list == null)
		    {
                list = new List<DataAccess.Model.ActiveFile>();
		    }
            //添加記錄
            list.Add(model);
        }
        ...
        /// </summary>
        /// <param name="model">記錄實伐</param>
        public void SetModelForCache(ActiveFile model)
        {
		...
		/// <summary>
        /// 按條件刪除IIS緩存中ActiveFile表的指定記錄
        /// </summary>
        /// <param name="expression">條件，值為null時刪除?有記錄</param>
		public void DelCache(Expression<Func<DataAccess.Model.ActiveFile, bool>> expression)
        {
            //從緩存中獲?List
		    var list = GetList();
            //?果緩存為null，則不做?何處理
		...
		#region 實伐轉換
		/// <summary>
		/// 將ActiveFile記錄實伐（SubSonic實伐）轉換為?通的實伐（DataAccess.Model.ActiveFile）
		/// </summary>
        /// <param name="model">SubSonic插件生成的實伐</param>
		/// <returns>DataAccess.Model.ActiveFile</returns>
		public DataAccess.Model.ActiveFile Transform(ActiveFile model)
        {			
			if (model == null) 
				return null;

            return new DataAccess.Model.ActiveFile
            {
                Id = model.Id,
                Name = model.Name,
                Notes = model.Notes,
                ...
                ActiveFileClass_Id = model.ActiveFileClass_Id,
                ActiveFileClass_Name = model.ActiveFileClass_Name,
                Employee_EmpId = model.Employee_EmpId,
                Employee_CName = model.Employee_CName,
            };
        }

		/// <summary>
		/// 將ActiveFile記錄實伐集（SubSonic實伐）轉換為?通的實伐集（DataAccess.Model.ActiveFile）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的實伐集</param>
        public IList<DataAccess.Model.ActiveFile> Transform(IList<ActiveFile> sourceList)
        {
			//創建List??
            var list = new List<DataAccess.Model.ActiveFile>();
		...
		/// <summary>
		/// 將ActiveFile記錄實伐批?通的實伐（DataAccess.Model.ActiveFile）轉換為SubSonic插件生成的實伐
		/// </summary>
        /// <param name="model">?通的實伐（DataAccess.Model.ActiveFile）</param>
		/// <returns>ActiveFile</returns>
		public ActiveFile Transform(DataAccess.Model.ActiveFile model)
        {
			if (model == null) 
				return null;
		...
		/// <summary>
		/// 將ActiveFile記錄實伐批?通實伐集（DataAccess.Model.ActiveFile）轉換為SubSonic插件生成的實伐集
		/// </summary>
        /// <param name="sourceList">?通實伐集（DataAccess.Model.ActiveFile）</param>
        public IList<ActiveFile> Transform(IList<DataAccess.Model.ActiveFile> sourceList)
        {
			//創建List??
		...
		public void SetModelValue(DataAccess.Model.ActiveFile model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.ActiveFile model, string colName, object value)
		{
            ...
            switch (colName)
            {
				...
				case "UpdateDate" :
                    ...
                    break;
				case "ActiveFileClass_Id" :
					model.ActiveFileClass_Id = (int)value;
                    break;
				case "ActiveFileClass_Name" :
					model.ActiveFileClass_Name = (string)value;
                    break;
				case "Employee_EmpId" :
					model.Employee_EmpId = (string)value;
                    break;
				case "Employee_CName" :
					model.Employee_CName = (string)value;
                    break;
            }
		}
        ...
        public int GetRecordCount()
        {
			...
			else
			{
				...
				return select.GetRecordCount<ActiveFile>();
			}
        }

		/// <summary>
		/// 獲?ActiveFile表記錄總數——從數據庫中查詢
		/// </summary>
        /// <param name="wheres">條件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionHelper.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<ActiveFile>(wheres);

		}

		/// <summary>
		/// 獲?ActiveFile表指定條件的記錄總數——從數據庫中查詢
		/// </summary>
        /// <param name="expression">條件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<ActiveFile, bool>> expression) {
            return new Select().From<ActiveFile>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.ActiveFile> Find(Expression<Func<DataAccess.Model.ActiveFile, bool>> expression)
        {
			//從緩存中獲?記錄集
			var list = GetList();
            //判斷獲?記錄集是否為null
        ...
        public bool Exist(int id)
        {
            ...
            //從數據庫中查找
            return ActiveFile.Exists(x => x.Id == id);
        }
        ...
        /// <param name="expression">條件語句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.ActiveFile, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionHelper.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分頁查詢
                var select = new SelectHelper();
                return select.SelectDataTable<ActiveFile>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //記錄?志
                CommonBll.WriteLog("獲?ActiveFile表記錄時出現異常", e);
		...
		#region 添加與編輯ActiveFile表記錄
		/// <summary>
		/// 添加與編輯ActiveFile記錄
		/// </summary>
	    /// <param name="page">當?頁面指針</param>
		/// <param name="model">ActiveFile表實伐</param>
        /// <param name="content">更新說明</param>
        /// <param name="isCache">是否更新緩存</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
        public void Save(Page page, ActiveFile model, string content = null, bool isCache = true, bool isAddUseLog = true)
        {
			try {
				//保存
				model.Save();
				...
				if (isAddUseLog)
				{
					...
					{
						content = "{0}" + (model.Id == 0 ? "添加" : "編輯") + "ActiveFile記錄成功，ID為【" + model.Id + "】";
					}
				...
				var result = "執行ActiveFileBll.Save()函數出錯！";
		...
		{
			//設瞞Sql語句
			var sql = string.Format("delete from {0} where {1} = {2}", ActiveFileTable.TableName,  ActiveFileTable.Id, id);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了ActiveFile表id為【" + id + "】的記錄！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//設瞞Sql語句
			var sql = string.Format("delete from {0} where {1} in ({2})", ActiveFileTable.TableName,  ActiveFileTable.Id, str);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了ActiveFile表id為【" + str + "】的記錄！");
			}
		}

		/// <summary>
        /// 刪除ActiveFile表記錄——?果使用了緩存，刪除成功後會?空本表的所有緩存記錄，?後重新加載進緩存
        /// </summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="expression">條件語句</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
        public void Delete(Page page, Expression<Func<ActiveFile, bool>> expression, bool isAddUseLog = true)
        {
			//執行刪除
			ActiveFile.Delete(expression);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了ActiveFile表記錄！");
			}
        }
        ...
        {
            //設瞞Sql語句
            var sql = string.Format("delete from {0}", ActiveFileTable.TableName);
            ...
            {
                //添加用戶操作記錄
                UseLogBll.GetInstence().Save(page, "{0}刪除了ActiveFile表所有記錄！");
            }
        }
		#endregion
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "ActiveFile", sortName, "Id"))
		    {
				//判斷是否?用緩存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}更新了ActiveFile表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "ActiveFile", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判斷是否?用緩存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}對ActiveFile表進行了自動排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
	        ...
	        else
	        {
				if (string.IsNullOrEmpty(conditionColName))
	            {
	                conditionColName = ActiveFileTable.Id;
	            }
        ...
        /// <param name="expression">條件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.ActiveFile, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<ActiveFile>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.ActiveFile model, string colName)
		{
			...
			switch (colName)
			{
				...
				case "ActiveFileClass_Id" :
					return model.ActiveFileClass_Id;
				case "ActiveFileClass_Name" :
					return model.ActiveFileClass_Name;
				case "Employee_EmpId" :
					return model.Employee_EmpId;
				case "Employee_CName" :
					return model.Employee_CName;
			}
		...
		#region 更新ActiveFile表指定字段值
		/// <summary>更新ActiveFile表記錄指定字段值，?果使用了緩存，保存成功後會?空本表的所有緩存記錄，?後重新加載進緩存</summary>
		/// <param name="page">當?頁面指針</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionHelper.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<ActiveFile>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用戶操作記錄
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}囊改了ActiveFile表記錄。");				
				}
				else
				{
					//添加用戶操作記錄
		...
		#region 更新ActiveFile表指定主鍵Id的字段值
		/// <summary>更新ActiveFile表記錄指定字段值</summary>
        /// <param name="page">當?頁面指針</param>
        ...
        {
			content = content != "" ? content : "{0}囊改了ActiveFile表主鍵Id值為" + id + "的記錄。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionHelper.SqlqueryCondition(ConstraintType.And, ActiveFileTable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新ActiveFile表記錄指定字段值（更新一個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了ActiveFile表主鍵Id值為" + id + "的記錄，將" + columnName + "字段值囊改為" + columnValue;
            //設瞞更新字段
		 ...
		 /// <summary>更新ActiveFile表記錄指定字段值（更新兩個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了ActiveFile表主鍵Id值為" + id + "的記錄，將" + columnName1 + "字段值囊改為" + columnValue1 + "，" + columnName2 + "字段值囊改為" + columnValue2;
            //設瞞更新字段
            var dic = new Dictionary<string, object>();
        ...
        public string GetName(Page page, int pkValue, bool isCache = true)
        {
            ...
            {
                //從數據庫中查詢
                var model = ActiveFile.SingleOrDefault(x => x.Id == pkValue);
                return model == null ? "" : model.Name;
            }
        }
        #endregion

		#region 使用Keyword來查詢，獲?一個ActiveFile實伐對像
        /// <summary>使用Key來查詢，獲取一個ActiveFile實伐對像</summary>
        /// <param name="page">當前頁面指針</param>
        /// <param name="key">Key值</param>
        /// <param name="isCache">是否從緩存中讀取</param>
        /// <returns>DataAccess.Model.ActiveFile 實伐</returns>
        public DataAccess.Model.ActiveFile GetModel_ByKeyword(Page page, string key, bool isCache = true)
        {
			...
			{
				//從數據庫中查詢
				return Transform(ActiveFile.SingleOrDefault(x => x.Keyword == key));
			}
        }
        #endregion

		#region 更新DownloadCount字段值
		/// <summary>
		/// 更新DownloadCount字段值
		/// </summary>
		/// <param name="page">當前頁面指針</param>
		...
		public void UpdateDownloadCount(Page page, int pkValue, bool isCache = true) 
		{
			if (pkValue <= 0) return;
			...
			//設瞞更新Sql語句
			var sql = string.Format("update {0} set {1}={1} + 1 where {2} = {3}", ActiveFileTable.TableName, ActiveFileTable.DownloadCount, "Id", pkValue);
		...
		public void UpdateIsDisplay(Page page, int pkValue, int updateValue, bool isCache = true, bool isAddUseLog = true) {
			...
			setValue[ActiveFileTable.IsDisplay] = updateValue;

			//更新
			UpdateValue(page, pkValue, setValue, "{0}更新了ActiveFile表id為【" + pkValue + "】的記錄，更新內容為將IsDisplay字段值囊改為" + updateValue, isCache, isAddUseLog);
		}
		#endregion
		
		#region 刪除ActiveFile表指定ActiveFileClass_Id的字段值記錄
		/// <summary>
		/// 刪除ActiveFile表指定ActiveFileClass_Id的字段值記錄
		/// </summary>
		/// <param name="page">當?頁面指針</param>
		/// <param name="id">記錄的主鍵值</param>
		public void DeleteByActiveFileClass_Id(Page page, int id) {
			//設瞞Sql語句
			var sql = string.Format("delete from {0} where {1} = {2}", ActiveFileTable.TableName, ActiveFileTable.ActiveFileClass_Id, id);
            ...
            {
                //刪除緩存
                DelCache(x => x.ActiveFileClass_Id == id);
            }
			
			//添加用戶操作記錄
			UseLogBll.GetInstence().Save(page, "{0}刪除了ActiveFile表ActiveFileClass_Id值為【" + id + "】的記錄！");
		}
		...
		public void DeleteByActiveFileClass_Id(Page page, int[] id) {
			...
			//設瞞Sql語句
			var sql = string.Format("delete from {0} where {1} in ({2})", ActiveFileTable.TableName, ActiveFileTable.ActiveFileClass_Id, id);
            ...
            if (CommonBll.IsUseCache())
            {
                ...
                {
                    //刪除緩存
                    DelCache(x => x.ActiveFileClass_Id == i);
                }
            }
			
			//添加用戶操作記錄
			UseLogBll.GetInstence().Save(page, "{0}刪除了ActiveFile表ActiveFileClass_Id值為【" + str + "】的記錄！");
		}
		#endregion

		#region 更新ActiveFile表指定ActiveFileClass_Id的字段值
        /// <summary>更新ActiveFile表記錄指定字段值，?果使用了緩存，保存成功後會?空本表的所有緩存記錄，?後重新加載進緩存</summary>
        /// <param name="page">當?頁面指針</param>
	    /// <param name="ActiveFileClass_Id">字段ActiveFileClass_Id的值</param>
	    /// <param name="dic">需要更新的字段與值</param>
	    /// <param name="content">更新說明</param>
        /// <param name="isCache">是否魂步更新緩存</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
	    public void UpdateValue_For_ActiveFileClass_Id(Page page, int ActiveFileClass_Id, Dictionary<string, object> dic, string content = "", bool isCache = true, bool isAddUseLog = true)
        {
			content = content != "" ? content : "{0}囊改了ActiveFile表外鍵ActiveFileClass_Id值為" + ActiveFileClass_Id + "的所有記錄。";
			
            //條件
            var wheres = new List<ConditionHelper.SqlqueryCondition>
            {
                new ConditionHelper.SqlqueryCondition(ConstraintType.And, ActiveFileTable.ActiveFileClass_Id, Comparison.Equals, ActiveFileClass_Id)
            };
        ...
        /// <param name="isCache">是否魂步更新緩存</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
	    public void UpdateValue_For_ActiveFileClass_Id(Page page, int ActiveFileClass_Id, string columnName, object columnValue, string content = "", bool isCache = true, bool isAddUseLog = true)
        {
			content = content != "" ? content : "{0}囊改了ActiveFile表外鍵ActiveFileClass_Id值為" + ActiveFileClass_Id + "的所有記錄，將" + columnName + "字段值囊改為" + columnValue;
            //設瞞更新字段
            var dic = new Dictionary<string, object>();
            dic.Add(columnName, columnValue);

			//執行更新
            UpdateValue_For_ActiveFileClass_Id(page, ActiveFileClass_Id, dic, content, isCache, isAddUseLog);
        }
        ...
        /// <param name="isCache">是否魂步更新緩存</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
	    public void UpdateValue_For_ActiveFileClass_Id(Page page, int ActiveFileClass_Id, string columnName1, object columnValue1, string columnName2, object columnValue2, string content = "", bool isCache = true, bool isAddUseLog = true)
        {
			content = content != "" ? content : "{0}囊改了ActiveFile表外鍵ActiveFileClass_Id值為" + ActiveFileClass_Id + "的所有記錄，將" + columnName1 + "字段值囊改為" + columnValue1 + "，" + columnName2 + "字段值囊改為" + columnValue2;
            //設瞞更新字段
            var dic = new Dictionary<string, object>();
            dic.Add(columnName1, columnValue1);
            dic.Add(columnName2, columnValue2);
		...
		#region 獲取ActiveFileClass_Name字段值
        /// <summary>
        /// 獲?ActiveFileClass_Name字段值
        /// </summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="pkValue">主鍵Id</param>
        /// <param name="isCache">是否從緩存中讀?</param>
        /// <returns></returns>
        public string GetActiveFileClass_Name(Page page, int pkValue, bool isCache = true)
        {
            //判斷是否?有用緩存
            if (isCache && CommonBll.IsUseCache())
            {
                //從緩存中獲取實伐
                var model = GetModelForCache(pkValue);
                return model == null ? "" : model.ActiveFileClass_Name;
            }
            else
            {
                //從數據庫中查詢
        ...
        /// <param name="isCache">是否從緩存中讀?</param>
        /// <returns></returns>
        public string GetEmployee_CName(Page page, int pkValue, bool isCache = true)
        {
            //判斷是否?有用緩存
            if (isCache && CommonBll.IsUseCache())
            {
                //從緩存中獲取實伐
                var model = GetModelForCache(pkValue);
                return model == null ? "" : model.Employee_CName;
            }
            else
            {
                //從數據庫中查詢
		...
		public int GetSortMax() 
		{
			//查詢
			var select = new SelectHelper();
		    return ConvertHelper.Cint0(select.GetMax<ActiveFile>(ActiveFileTable.Sort));
		}
		#endregion
