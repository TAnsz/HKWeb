...
namespace Solution.Logic.Managers {
	...
	public partial class MeetingRoomBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_MeetingRoom";
        private const string const_CacheKey_Date = "Cache_MeetingRoom_Date";

		#region 單例模式
		//定義單例實伐
		private static MeetingRoomBll _MeetingRoomBll = null;

		/// <summary>
		/// 獲取本邏輯類單例
		/// </summary>
		/// <returns></returns>
		public static MeetingRoomBll GetInstence() {
			if (_MeetingRoomBll == null) {
				_MeetingRoomBll = new MeetingRoomBll();
			}
			return _MeetingRoomBll;
		}
		#endregion
		...
		#region 從IIS緩存中獲取MeetingRoom表記錄
		/// <summary>
        /// 從IIS緩存中獲取MeetingRoom表記錄
        /// </summary>
	    /// <param name="isCache">是否從緩存中讀取</param>
        public IList<DataAccess.Model.MeetingRoom> GetList(bool isCache = true)
        {
			try
			{
				//判斷是否使用緩存
				if (CommonBll.IsUseCache() && isCache){
					...
					{
						return (IList<DataAccess.Model.MeetingRoom>)obj;
					}
				}
				else
				{
					//定義臨時實伐集
					IList<DataAccess.Model.MeetingRoom> list = null;

					//獲??表緩存加載條件表達式
					var exp = GetExpression<MeetingRoom>();
                    //?果條件為空，則查詢?表所有記錄
					if (exp == null)
					{
						//從數據庫中獲?所有記錄
						var all = MeetingRoom.All();
                        list = all == null ? null : Transform(all.ToList());
					}
					else
					{
                        //從數據庫中查詢出指定條件的記錄，並轉換為指定實伐集
						var all = MeetingRoom.Find(exp);
            ...
            {
                //記錄?志
                CommonBll.WriteLog("從IIS緩存中獲取MeetingRoom表記錄時出現異常", e);
			}
		...
		/// <returns>DataAccess.Model.MeetingRoom</returns>
        public DataAccess.Model.MeetingRoom GetModel(long id, bool isCache = true)
        {
            //判斷是否使用緩存
		    if (CommonBll.IsUseCache() && isCache)
		    {
		    ...
		    {
                //從數據庫中直接讀?
                var model = MeetingRoom.SingleOrDefault(x => x.Id == id);
                if (model == null)
                {
                    return null;
                }
        ...
        /// <param name="id">主鍵Id</param>
		/// <returns>DataAccess.Model.MeetingRoom</returns>
        public DataAccess.Model.MeetingRoom GetModelForCache(long id)
        {
			try
			{
				//從緩存中讀?指定Id記錄
				...
				if (model == null){
					//從數據庫中讀?
					var tem = MeetingRoom.SingleOrDefault(x => x.Id == id);
					if (tem == null)
					{
						return null;
					}
        ...
        /// <returns>DataAccess.Model.MeetingRoom</returns>
        public DataAccess.Model.MeetingRoom GetModelForCache(string conditionColName, object value)
        {
			try
            {
                //從緩存中獲取List
                var list = GetList();
                DataAccess.Model.MeetingRoom model = null;
                Expression<Func<MeetingRoom, bool>> expression = null;
                ...
                switch (conditionColName)
                {
					...
					case "Name" :
                        ...
                        break;
					case "Qty" :
						model = list.SingleOrDefault(x => x.Qty == (int)value);
                        expression = x => x.Qty == (int)value;
                        break;
					case "Address" :
                ...
                {
                    //從數據庫中讀取
                    var tem = MeetingRoom.SingleOrDefault(expression);
                    if (tem == null)
                    {
                        return null;
                    }
        ...
        /// <returns>DataAccess.Model.MeetingRoom</returns>
        public DataAccess.Model.MeetingRoom GetModelForCache(Expression<Func<DataAccess.Model.MeetingRoom, bool>> expression)
        {
			//從緩存中讀?記錄列表
			var list = GetList();
            //?果條件為空，則查詢?表所有記錄
        ...
        public void SetModelForCache(DataAccess.Model.MeetingRoom model)
        {
		    ...
		    if (list == null)
		    {
                list = new List<DataAccess.Model.MeetingRoom>();
		    }
            //添加記錄
            list.Add(model);
        }
        ...
        /// </summary>
        /// <param name="model">記錄實伐</param>
        public void SetModelForCache(MeetingRoom model)
        {
		...
		/// <summary>
        /// 按條件刪除IIS緩存中MeetingRoom表的指定記錄
        /// </summary>
        /// <param name="expression">條件，值為null時刪除?有記錄</param>
		public void DelCache(Expression<Func<DataAccess.Model.MeetingRoom, bool>> expression)
        {
            //從緩存中獲?List
		    var list = GetList();
            //?果緩存為null，則不做?何處理
		...
		#region 實伐轉換
		/// <summary>
		/// 將MeetingRoom記錄實伐（SubSonic實伐）轉換為?通的實伐（DataAccess.Model.MeetingRoom）
		/// </summary>
        /// <param name="model">SubSonic插件生成的實伐</param>
		/// <returns>DataAccess.Model.MeetingRoom</returns>
		public DataAccess.Model.MeetingRoom Transform(MeetingRoom model)
        {			
			if (model == null) 
				return null;

            return new DataAccess.Model.MeetingRoom
            {
                Id = model.Id,
                Code = model.Code,
                Name = model.Name,
		...
		/// <summary>
		/// 將MeetingRoom記錄實伐集（SubSonic實伐）轉換為?通的實伐集（DataAccess.Model.MeetingRoom）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的實伐集</param>
        public IList<DataAccess.Model.MeetingRoom> Transform(IList<MeetingRoom> sourceList)
        {
			//創建List??
            var list = new List<DataAccess.Model.MeetingRoom>();
		...
		/// <summary>
		/// 將MeetingRoom記錄實伐批?通的實伐（DataAccess.Model.MeetingRoom）轉換為SubSonic插件生成的實伐
		/// </summary>
        /// <param name="model">?通的實伐（DataAccess.Model.MeetingRoom）</param>
		/// <returns>MeetingRoom</returns>
		public MeetingRoom Transform(DataAccess.Model.MeetingRoom model)
        {
			if (model == null) 
				return null;
		...
		/// <summary>
		/// 將MeetingRoom記錄實伐批?通實伐集（DataAccess.Model.MeetingRoom）轉換為SubSonic插件生成的實伐集
		/// </summary>
        /// <param name="sourceList">?通實伐集（DataAccess.Model.MeetingRoom）</param>
        public IList<MeetingRoom> Transform(IList<DataAccess.Model.MeetingRoom> sourceList)
        {
			//創建List??
		...
		public void SetModelValue(DataAccess.Model.MeetingRoom model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.MeetingRoom model, string colName, object value)
		{
        ...
        public int GetRecordCount()
        {
			...
			else
			{
				...
				return select.GetRecordCount<MeetingRoom>();
			}
        }

		/// <summary>
		/// 獲?MeetingRoom表記錄總數——從數據庫中查詢
		/// </summary>
        /// <param name="wheres">條件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionHelper.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<MeetingRoom>(wheres);

		}

		/// <summary>
		/// 獲?MeetingRoom表指定條件的記錄總數——從數據庫中查詢
		/// </summary>
        /// <param name="expression">條件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<MeetingRoom, bool>> expression) {
            return new Select().From<MeetingRoom>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.MeetingRoom> Find(Expression<Func<DataAccess.Model.MeetingRoom, bool>> expression)
        {
			//從緩存中獲?記錄集
			var list = GetList();
            //判斷獲?記錄集是否為null
        ...
        public bool Exist(int id)
        {
            ...
            //從數據庫中查找
            return MeetingRoom.Exists(x => x.Id == id);
        }
        ...
        /// <param name="expression">條件語句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.MeetingRoom, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionHelper.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分頁查詢
                var select = new SelectHelper();
                return select.SelectDataTable<MeetingRoom>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //記錄?志
                CommonBll.WriteLog("獲?MeetingRoom表記錄時出現異常", e);
		...
		#region 添加與編輯MeetingRoom表記錄
		/// <summary>
		/// 添加與編輯MeetingRoom記錄
		/// </summary>
	    /// <param name="page">當?頁面指針</param>
		/// <param name="model">MeetingRoom表實伐</param>
        /// <param name="content">更新說明</param>
        /// <param name="isCache">是否更新緩存</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
        public void Save(Page page, MeetingRoom model, string content = null, bool isCache = true, bool isAddUseLog = true)
        {
			try {
				//保存
				model.Save();
				...
				if (isAddUseLog)
				{
					...
					{
						content = "{0}" + (model.Id == 0 ? "添加" : "編輯") + "MeetingRoom記錄成功，ID為【" + model.Id + "】";
					}
				...
				var result = "執行MeetingRoomBll.Save()函數出錯！";
		...
		{
			//設瞞Sql語句
			var sql = string.Format("delete from {0} where {1} = {2}", MeetingRoomTable.TableName,  MeetingRoomTable.Id, id);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了MeetingRoom表id為【" + id + "】的記錄！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//設瞞Sql語句
			var sql = string.Format("delete from {0} where {1} in ({2})", MeetingRoomTable.TableName,  MeetingRoomTable.Id, str);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了MeetingRoom表id為【" + str + "】的記錄！");
			}
		}

		/// <summary>
        /// 刪除MeetingRoom表記錄——?果使用了緩存，刪除成功後會?空本表的所有緩存記錄，?後重新加載進緩存
        /// </summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="expression">條件語句</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
        public void Delete(Page page, Expression<Func<MeetingRoom, bool>> expression, bool isAddUseLog = true)
        {
			//執行刪除
			MeetingRoom.Delete(expression);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了MeetingRoom表記錄！");
			}
        }
        ...
        {
            //設瞞Sql語句
            var sql = string.Format("delete from {0}", MeetingRoomTable.TableName);
            ...
            {
                //添加用戶操作記錄
                UseLogBll.GetInstence().Save(page, "{0}刪除了MeetingRoom表所有記錄！");
            }
        }
		#endregion
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "MeetingRoom", sortName, "Id"))
		    {
				//判斷是否?用緩存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}更新了MeetingRoom表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "MeetingRoom", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判斷是否?用緩存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}對MeetingRoom表進行了自動排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
	        ...
	        else
	        {
				if (string.IsNullOrEmpty(conditionColName))
	            {
	                conditionColName = MeetingRoomTable.Id;
	            }
        ...
        /// <param name="expression">條件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.MeetingRoom, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<MeetingRoom>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.MeetingRoom model, string colName)
		{
		...
		#region 更新MeetingRoom表指定字段值
		/// <summary>更新MeetingRoom表記錄指定字段值，?果使用了緩存，保存成功後會?空本表的所有緩存記錄，?後重新加載進緩存</summary>
		/// <param name="page">當?頁面指針</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionHelper.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<MeetingRoom>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用戶操作記錄
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}囊改了MeetingRoom表記錄。");				
				}
				else
				{
					//添加用戶操作記錄
		...
		#region 更新MeetingRoom表指定主鍵Id的字段值
		/// <summary>更新MeetingRoom表記錄指定字段值</summary>
        /// <param name="page">當?頁面指針</param>
        ...
        {
			content = content != "" ? content : "{0}囊改了MeetingRoom表主鍵Id值為" + id + "的記錄。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionHelper.SqlqueryCondition(ConstraintType.And, MeetingRoomTable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新MeetingRoom表記錄指定字段值（更新一個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了MeetingRoom表主鍵Id值為" + id + "的記錄，將" + columnName + "字段值囊改為" + columnValue;
            //設瞞更新字段
		 ...
		 /// <summary>更新MeetingRoom表記錄指定字段值（更新兩個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了MeetingRoom表主鍵Id值為" + id + "的記錄，將" + columnName1 + "字段值囊改為" + columnValue1 + "，" + columnName2 + "字段值囊改為" + columnValue2;
            //設瞞更新字段
            var dic = new Dictionary<string, object>();
        ...
        public string GetName(Page page, int pkValue, bool isCache = true)
        {
            ...
            {
                //從數據庫中查詢
                var model = MeetingRoom.SingleOrDefault(x => x.Id == pkValue);
                return model == null ? "" : model.Name;
            }
        }
        #endregion
		...
		public void UpdateIsVaild(Page page, int pkValue, int updateValue, bool isCache = true, bool isAddUseLog = true) {
			...
			setValue[MeetingRoomTable.IsVaild] = updateValue;

			//更新
			UpdateValue(page, pkValue, setValue, "{0}更新了MeetingRoom表id為【" + pkValue + "】的記錄，更新內容為將IsVaild字段值囊改為" + updateValue, isCache, isAddUseLog);
		}
		#endregion
