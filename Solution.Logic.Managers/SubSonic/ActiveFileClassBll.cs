...
namespace Solution.Logic.Managers {
	...
	public partial class ActiveFileClassBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_ActiveFileClass";
        private const string const_CacheKey_Date = "Cache_ActiveFileClass_Date";

		#region 單例模式
		//定義單例實伐
		private static ActiveFileClassBll _ActiveFileClassBll = null;

		/// <summary>
		/// 獲取本邏輯類單例
		/// </summary>
		/// <returns></returns>
		public static ActiveFileClassBll GetInstence() {
			if (_ActiveFileClassBll == null) {
				_ActiveFileClassBll = new ActiveFileClassBll();
			}
			return _ActiveFileClassBll;
		}
		#endregion
		...
		#region 從IIS緩存中獲取ActiveFileClass表記錄
		/// <summary>
        /// 從IIS緩存中獲取ActiveFileClass表記錄
        /// </summary>
	    /// <param name="isCache">是否從緩存中讀取</param>
        public IList<DataAccess.Model.ActiveFileClass> GetList(bool isCache = true)
        {
			try
			{
				//判斷是否使用緩存
				if (CommonBll.IsUseCache() && isCache){
					...
					{
						return (IList<DataAccess.Model.ActiveFileClass>)obj;
					}
				}
				else
				{
					//定義臨時實伐集
					IList<DataAccess.Model.ActiveFileClass> list = null;

					//獲??表緩存加載條件表達式
					var exp = GetExpression<ActiveFileClass>();
                    //?果條件為空，則查詢?表所有記錄
					if (exp == null)
					{
						//從數據庫中獲?所有記錄
						var all = ActiveFileClass.All();
                        list = all == null ? null : Transform(all.ToList());
					}
					else
					{
                        //從數據庫中查詢出指定條件的記錄，並轉換為指定實伐集
						var all = ActiveFileClass.Find(exp);
            ...
            {
                //記錄?志
                CommonBll.WriteLog("從IIS緩存中獲取ActiveFileClass表記錄時出現異常", e);
			}
		...
		/// <returns>DataAccess.Model.ActiveFileClass</returns>
        public DataAccess.Model.ActiveFileClass GetModel(long id, bool isCache = true)
        {
            //判斷是否使用緩存
		    if (CommonBll.IsUseCache() && isCache)
		    {
		    ...
		    {
                //從數據庫中直接讀?
                var model = ActiveFileClass.SingleOrDefault(x => x.Id == id);
                if (model == null)
                {
                    return null;
                }
        ...
        /// <param name="id">主鍵Id</param>
		/// <returns>DataAccess.Model.ActiveFileClass</returns>
        public DataAccess.Model.ActiveFileClass GetModelForCache(long id)
        {
			try
			{
				//從緩存中讀?指定Id記錄
				...
				if (model == null){
					//從數據庫中讀?
					var tem = ActiveFileClass.SingleOrDefault(x => x.Id == id);
					if (tem == null)
					{
						return null;
					}
        ...
        /// <returns>DataAccess.Model.ActiveFileClass</returns>
        public DataAccess.Model.ActiveFileClass GetModelForCache(string conditionColName, object value)
        {
			try
            {
                //從緩存中獲取List
                var list = GetList();
                DataAccess.Model.ActiveFileClass model = null;
                Expression<Func<ActiveFileClass, bool>> expression = null;
                ...
                {
                    //從數據庫中讀取
                    var tem = ActiveFileClass.SingleOrDefault(expression);
                    if (tem == null)
                    {
                        return null;
                    }
        ...
        /// <returns>DataAccess.Model.ActiveFileClass</returns>
        public DataAccess.Model.ActiveFileClass GetModelForCache(Expression<Func<DataAccess.Model.ActiveFileClass, bool>> expression)
        {
			//從緩存中讀?記錄列表
			var list = GetList();
            //?果條件為空，則查詢?表所有記錄
        ...
        public void SetModelForCache(DataAccess.Model.ActiveFileClass model)
        {
		    ...
		    if (list == null)
		    {
                list = new List<DataAccess.Model.ActiveFileClass>();
		    }
            //添加記錄
            list.Add(model);
        }
        ...
        /// </summary>
        /// <param name="model">記錄實伐</param>
        public void SetModelForCache(ActiveFileClass model)
        {
		...
		/// <summary>
        /// 按條件刪除IIS緩存中ActiveFileClass表的指定記錄
        /// </summary>
        /// <param name="expression">條件，值為null時刪除?有記錄</param>
		public void DelCache(Expression<Func<DataAccess.Model.ActiveFileClass, bool>> expression)
        {
            //從緩存中獲?List
		    var list = GetList();
            //?果緩存為null，則不做?何處理
		...
		#region 實伐轉換
		/// <summary>
		/// 將ActiveFileClass記錄實伐（SubSonic實伐）轉換為?通的實伐（DataAccess.Model.ActiveFileClass）
		/// </summary>
        /// <param name="model">SubSonic插件生成的實伐</param>
		/// <returns>DataAccess.Model.ActiveFileClass</returns>
		public DataAccess.Model.ActiveFileClass Transform(ActiveFileClass model)
        {			
			if (model == null) 
				return null;

            return new DataAccess.Model.ActiveFileClass
            {
                Id = model.Id,
                Name = model.Name,
		...
		/// <summary>
		/// 將ActiveFileClass記錄實伐集（SubSonic實伐）轉換為?通的實伐集（DataAccess.Model.ActiveFileClass）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的實伐集</param>
        public IList<DataAccess.Model.ActiveFileClass> Transform(IList<ActiveFileClass> sourceList)
        {
			//創建List??
            var list = new List<DataAccess.Model.ActiveFileClass>();
		...
		/// <summary>
		/// 將ActiveFileClass記錄實伐批?通的實伐（DataAccess.Model.ActiveFileClass）轉換為SubSonic插件生成的實伐
		/// </summary>
        /// <param name="model">?通的實伐（DataAccess.Model.ActiveFileClass）</param>
		/// <returns>ActiveFileClass</returns>
		public ActiveFileClass Transform(DataAccess.Model.ActiveFileClass model)
        {
			if (model == null) 
				return null;

            return new ActiveFileClass
            {
                Id = model.Id,
                Name = model.Name,
                ParentId = model.ParentId,
		...
		/// <summary>
		/// 將ActiveFileClass記錄實伐批?通實伐集（DataAccess.Model.ActiveFileClass）轉換為SubSonic插件生成的實伐集
		/// </summary>
        /// <param name="sourceList">?通實伐集（DataAccess.Model.ActiveFileClass）</param>
        public IList<ActiveFileClass> Transform(IList<DataAccess.Model.ActiveFileClass> sourceList)
        {
			//創建List??
            var list = new List<ActiveFileClass>();
		...
		public void SetModelValue(DataAccess.Model.ActiveFileClass model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.ActiveFileClass model, string colName, object value)
		{
		...
		#region 獲?ActiveFileClass表記錄總數
        /// <summary>
        /// 獲?ActiveFileClass表記錄總數
        /// </summary>
        ...
        public int GetRecordCount()
        {
			...
			else
			{
				...
				return select.GetRecordCount<ActiveFileClass>();
			}
        }

		/// <summary>
		/// 獲?ActiveFileClass表記錄總數——從數據庫中查詢
		/// </summary>
        /// <param name="wheres">條件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionHelper.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<ActiveFileClass>(wheres);

		}

		/// <summary>
		/// 獲?ActiveFileClass表指定條件的記錄總數——從數據庫中查詢
		/// </summary>
        /// <param name="expression">條件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<ActiveFileClass, bool>> expression) {
            return new Select().From<ActiveFileClass>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.ActiveFileClass> Find(Expression<Func<DataAccess.Model.ActiveFileClass, bool>> expression)
        {
			//從緩存中獲?記錄集
			var list = GetList();
            //判斷獲?記錄集是否為null
        ...
        public bool Exist(int id)
        {
            ...
            //從數據庫中查找
            return ActiveFileClass.Exists(x => x.Id == id);
        }
        ...
        /// <param name="expression">條件語句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.ActiveFileClass, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		#region 獲?ActiveFileClass表記錄
		/// <summary>
		/// 獲?ActiveFileClass表記錄
		/// </summary>
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionHelper.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分頁查詢
                var select = new SelectHelper();
                return select.SelectDataTable<ActiveFileClass>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //記錄?志
                CommonBll.WriteLog("獲?ActiveFileClass表記錄時出現異常", e);
		...
		#region 添加與編輯ActiveFileClass表記錄
		/// <summary>
		/// 添加與編輯ActiveFileClass記錄
		/// </summary>
	    /// <param name="page">當?頁面指針</param>
		/// <param name="model">ActiveFileClass表實伐</param>
        /// <param name="content">更新說明</param>
        /// <param name="isCache">是否更新緩存</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
        public void Save(Page page, ActiveFileClass model, string content = null, bool isCache = true, bool isAddUseLog = true)
        {
			try {
				//保存
				model.Save();
				...
				if (isAddUseLog)
				{
					...
					{
						content = "{0}" + (model.Id == 0 ? "添加" : "編輯") + "ActiveFileClass記錄成功，ID為【" + model.Id + "】";
					}
				...
				var result = "執行ActiveFileClassBll.Save()函數出錯！";
		...
		#region 刪除ActiveFileClass表記錄
		/// <summary>
		/// 刪除ActiveFileClass表記錄
		/// </summary>
		/// <param name="page">當?頁面指針</param>
		...
		{
			//設瞞Sql語句
			var sql = string.Format("delete from {0} where {1} = {2}", ActiveFileClassTable.TableName,  ActiveFileClassTable.Id, id);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了ActiveFileClass表id為【" + id + "】的記錄！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//設瞞Sql語句
			var sql = string.Format("delete from {0} where {1} in ({2})", ActiveFileClassTable.TableName,  ActiveFileClassTable.Id, str);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了ActiveFileClass表id為【" + str + "】的記錄！");
			}
		}

		/// <summary>
        /// 刪除ActiveFileClass表記錄——?果使用了緩存，刪除成功後會?空本表的所有緩存記錄，?後重新加載進緩存
        /// </summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="expression">條件語句</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
        public void Delete(Page page, Expression<Func<ActiveFileClass, bool>> expression, bool isAddUseLog = true)
        {
			//執行刪除
			ActiveFileClass.Delete(expression);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了ActiveFileClass表記錄！");
			}
        }

		/// <summary>
        /// 刪除ActiveFileClass表所有記錄
        /// </summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="isAddUseLog">是否添加用戶操作?志</param>
        ...
        {
            //設瞞Sql語句
            var sql = string.Format("delete from {0}", ActiveFileClassTable.TableName);
            ...
            {
                //添加用戶操作記錄
                UseLogBll.GetInstence().Save(page, "{0}刪除了ActiveFileClass表所有記錄！");
            }
        }
		#endregion
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "ActiveFileClass", sortName, "Id"))
		    {
				//判斷是否?用緩存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}更新了ActiveFileClass表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "ActiveFileClass", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判斷是否?用緩存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}對ActiveFileClass表進行了自動排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
	        ...
	        else
	        {
				if (string.IsNullOrEmpty(conditionColName))
	            {
	                conditionColName = ActiveFileClassTable.Id;
	            }
        ...
        /// <param name="expression">條件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.ActiveFileClass, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<ActiveFileClass>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.ActiveFileClass model, string colName)
		{
		...
		#region 更新ActiveFileClass表指定字段值
		/// <summary>更新ActiveFileClass表記錄指定字段值，?果使用了緩存，保存成功後會?空本表的所有緩存記錄，?後重新加載進緩存</summary>
		/// <param name="page">當?頁面指針</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionHelper.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<ActiveFileClass>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用戶操作記錄
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}囊改了ActiveFileClass表記錄。");				
				}
				else
				{
					//添加用戶操作記錄
		...
		#region 更新ActiveFileClass表指定主鍵Id的字段值
		/// <summary>更新ActiveFileClass表記錄指定字段值</summary>
        /// <param name="page">當?頁面指針</param>
        ...
        {
			content = content != "" ? content : "{0}囊改了ActiveFileClass表主鍵Id值為" + id + "的記錄。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionHelper.SqlqueryCondition(ConstraintType.And, ActiveFileClassTable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新ActiveFileClass表記錄指定字段值（更新一個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了ActiveFileClass表主鍵Id值為" + id + "的記錄，將" + columnName + "字段值囊改為" + columnValue;
            //設瞞更新字段
		 ...
		 /// <summary>更新ActiveFileClass表記錄指定字段值（更新兩個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了ActiveFileClass表主鍵Id值為" + id + "的記錄，將" + columnName1 + "字段值囊改為" + columnValue1 + "，" + columnName2 + "字段值囊改為" + columnValue2;
            //設瞞更新字段
            var dic = new Dictionary<string, object>();
        ...
        public string GetName(Page page, int pkValue, bool isCache = true)
        {
            ...
            {
                //從數據庫中查詢
                var model = ActiveFileClass.SingleOrDefault(x => x.Id == pkValue);
                return model == null ? "" : model.Name;
            }
        }
        #endregion

		#region 使用Keyword來查詢，獲?一個ActiveFileClass實伐對像
        /// <summary>使用Key來查詢，獲取一個ActiveFileClass實伐對像</summary>
        /// <param name="page">當前頁面指針</param>
        /// <param name="key">Key值</param>
        /// <param name="isCache">是否從緩存中讀取</param>
        /// <returns>DataAccess.Model.ActiveFileClass 實伐</returns>
        public DataAccess.Model.ActiveFileClass GetModel_ByKeyword(Page page, string key, bool isCache = true)
        {
			...
			{
				//從數據庫中查詢
				return Transform(ActiveFileClass.SingleOrDefault(x => x.Keyword == key));
			}
        }
        #endregion
		...
		public void UpdateIsDisplay(Page page, int pkValue, int updateValue, bool isCache = true, bool isAddUseLog = true) {
			...
			setValue[ActiveFileClassTable.IsDisplay] = updateValue;

			//更新
			UpdateValue(page, pkValue, setValue, "{0}更新了ActiveFileClass表id為【" + pkValue + "】的記錄，更新內容為將IsDisplay字段值囊改為" + updateValue, isCache, isAddUseLog);
		}
		#endregion
		...
		public int GetSortMax() 
		{
			//查詢
			var select = new SelectHelper();
		    return ConvertHelper.Cint0(select.GetMax<ActiveFileClass>(ActiveFileClassTable.Sort));
		}
		#endregion
		...
		public int GetSortMax(int parentId) 
		{
			//定義查詢條件
			var wheres = new List<ConditionHelper.SqlqueryCondition>();
			wheres.Add(new ConditionHelper.SqlqueryCondition(ConstraintType.And, ActiveFileClassTable.ParentId, Comparison.Equals, parentId));
			//查詢
			var select = new SelectHelper();
		    return ConvertHelper.Cint0(select.GetMax<ActiveFileClass>(ActiveFileClassTable.Sort, wheres));
		}
		#endregion
