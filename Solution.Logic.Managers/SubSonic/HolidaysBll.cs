...
namespace Solution.Logic.Managers {
	...
	/// Holidays表邏輯類
	/// </summary>
	public partial class HolidaysBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_Holidays";
        private const string const_CacheKey_Date = "Cache_Holidays_Date";

		#region 單例模式
		//定義單例實伐
		private static HolidaysBll _HolidaysBll = null;

		/// <summary>
		/// 獲取本邏輯類單例
		/// </summary>
		/// <returns></returns>
		public static HolidaysBll GetInstence() {
			if (_HolidaysBll == null) {
				_HolidaysBll = new HolidaysBll();
			}
			return _HolidaysBll;
		}
		#endregion
		...
		#region 從IIS緩存中獲取Holidays表記錄
		/// <summary>
        /// 從IIS緩存中獲取Holidays表記錄
        /// </summary>
	    /// <param name="isCache">是否從緩存中讀取</param>
        public IList<DataAccess.Model.Holidays> GetList(bool isCache = true)
        {
			try
			{
				//判斷是否使用緩存
				if (CommonBll.IsUseCache() && isCache){
					...
					{
						return (IList<DataAccess.Model.Holidays>)obj;
					}
				}
				else
				{
					//定義臨時實伐集
					IList<DataAccess.Model.Holidays> list = null;

					//獲??表緩存加載條件表達式
					var exp = GetExpression<Holidays>();
                    //?果條件為空，則查詢?表所有記錄
					if (exp == null)
					{
						//從數據庫中獲?所有記錄
						var all = Holidays.All();
                        list = all == null ? null : Transform(all.ToList());
					}
					else
					{
                        //從數據庫中查詢出指定條件的記錄，並轉換為指定實伐集
						var all = Holidays.Find(exp);
            ...
            {
                //記錄?志
                CommonBll.WriteLog("從IIS緩存中獲取Holidays表記錄時出現異常", e);
			}
		...
		/// <returns>DataAccess.Model.Holidays</returns>
        public DataAccess.Model.Holidays GetModel(long id, bool isCache = true)
        {
            //判斷是否使用緩存
		    if (CommonBll.IsUseCache() && isCache)
		    {
		    ...
		    {
                //從數據庫中直接讀?
                var model = Holidays.SingleOrDefault(x => x.Id == id);
                if (model == null)
                {
                    return null;
                }
        ...
        /// <param name="id">主鍵Id</param>
		/// <returns>DataAccess.Model.Holidays</returns>
        public DataAccess.Model.Holidays GetModelForCache(long id)
        {
			try
			{
				//從緩存中讀?指定Id記錄
				...
				if (model == null){
					//從數據庫中讀?
					var tem = Holidays.SingleOrDefault(x => x.Id == id);
					if (tem == null)
					{
						return null;
					}
        ...
        /// <returns>DataAccess.Model.Holidays</returns>
        public DataAccess.Model.Holidays GetModelForCache(string conditionColName, object value)
        {
			try
            {
                //從緩存中獲取List
                var list = GetList();
                DataAccess.Model.Holidays model = null;
                Expression<Func<Holidays, bool>> expression = null;
                ...
                switch (conditionColName)
                {
					case "Id" :
                        ...
                        break;
					case "hd_code" :
						model = list.SingleOrDefault(x => x.hd_code == (string)value);
                        expression = x => x.hd_code == (string)value;
                        break;
					case "hd_name" :
						model = list.SingleOrDefault(x => x.hd_name == (string)value);
                        expression = x => x.hd_name == (string)value;
                        break;
					case "hd_date" :
						model = list.SingleOrDefault(x => x.hd_date == (DateTime)value);
                        expression = x => x.hd_date == (DateTime)value;
                        break;
					case "hd_end" :
						model = list.SingleOrDefault(x => x.hd_end == (DateTime)value);
                        expression = x => x.hd_end == (DateTime)value;
                        break;
					case "begin_time" :
					...
					case "hd_rate" :
                        ...
                        break;
					case "hd_type" :
						model = list.SingleOrDefault(x => x.hd_type == (string)value);
                        expression = x => x.hd_type == (string)value;
                        break;
					case "hd_kind" :
						model = list.SingleOrDefault(x => x.hd_kind == (int)value);
                        expression = x => x.hd_kind == (int)value;
                        break;
					case "depart_id" :
					...
					case "alway_use" :
                        ...
                        break;
					case "sub_depart" :
						model = list.SingleOrDefault(x => x.sub_depart == (short)value);
                        expression = x => x.sub_depart == (short)value;
                        break;
					case "need_write" :
						model = list.SingleOrDefault(x => x.need_write == (short)value);
                        expression = x => x.need_write == (short)value;
                        break;
					case "have_hrs" :
                ...
                {
                    //從數據庫中讀取
                    var tem = Holidays.SingleOrDefault(expression);
                    if (tem == null)
                    {
                        return null;
                    }
        ...
        /// <returns>DataAccess.Model.Holidays</returns>
        public DataAccess.Model.Holidays GetModelForCache(Expression<Func<DataAccess.Model.Holidays, bool>> expression)
        {
			//從緩存中讀?記錄列表
			var list = GetList();
            //?果條件為空，則查詢?表所有記錄
        ...
        public void SetModelForCache(DataAccess.Model.Holidays model)
        {
		    ...
		    if (list == null)
		    {
                list = new List<DataAccess.Model.Holidays>();
		    }
            //添加記錄
            list.Add(model);
        }
        ...
        /// </summary>
        /// <param name="model">記錄實伐</param>
        public void SetModelForCache(Holidays model)
        {
		...
		/// <summary>
        /// 按條件刪除IIS緩存中Holidays表的指定記錄
        /// </summary>
        /// <param name="expression">條件，值為null時刪除?有記錄</param>
		public void DelCache(Expression<Func<DataAccess.Model.Holidays, bool>> expression)
        {
            //從緩存中獲?List
		    var list = GetList();
            //?果緩存為null，則不做?何處理
		...
		#region 實伐轉換
		/// <summary>
		/// 將Holidays記錄實伐（SubSonic實伐）轉換為?通的實伐（DataAccess.Model.Holidays）
		/// </summary>
        /// <param name="model">SubSonic插件生成的實伐</param>
		/// <returns>DataAccess.Model.Holidays</returns>
		public DataAccess.Model.Holidays Transform(Holidays model)
        {			
			if (model == null) 
				return null;

            return new DataAccess.Model.Holidays
            {
                Id = model.Id,
                hd_code = model.hd_code,
                hd_name = model.hd_name,
                hd_date = model.hd_date,
                hd_end = model.hd_end,
                begin_time = model.begin_time,
                end_time = model.end_time,
                hd_rate = model.hd_rate,
                hd_type = model.hd_type,
                hd_kind = model.hd_kind,
                depart_id = model.depart_id,
                alway_use = model.alway_use,
                sub_depart = model.sub_depart,
                need_write = model.need_write,
		...
		/// <summary>
		/// 將Holidays記錄實伐集（SubSonic實伐）轉換為?通的實伐集（DataAccess.Model.Holidays）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的實伐集</param>
        public IList<DataAccess.Model.Holidays> Transform(IList<Holidays> sourceList)
        {
			//創建List??
            var list = new List<DataAccess.Model.Holidays>();
		...
		/// <summary>
		/// 將Holidays記錄實伐批?通的實伐（DataAccess.Model.Holidays）轉換為SubSonic插件生成的實伐
		/// </summary>
        /// <param name="model">?通的實伐（DataAccess.Model.Holidays）</param>
		/// <returns>Holidays</returns>
		public Holidays Transform(DataAccess.Model.Holidays model)
        {
			if (model == null) 
				return null;
		...
		/// <summary>
		/// 將Holidays記錄實伐批?通實伐集（DataAccess.Model.Holidays）轉換為SubSonic插件生成的實伐集
		/// </summary>
        /// <param name="sourceList">?通實伐集（DataAccess.Model.Holidays）</param>
        public IList<Holidays> Transform(IList<DataAccess.Model.Holidays> sourceList)
        {
			//創建List??
		...
		public void SetModelValue(DataAccess.Model.Holidays model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.Holidays model, string colName, object value)
		{
            ...
            switch (colName)
            {
				case "Id" :
                    ...
                    break;
				case "hd_code" :
					model.hd_code = (string)value;
                    break;
				case "hd_name" :
					model.hd_name = (string)value;
                    break;
				case "hd_date" :
					model.hd_date = (DateTime)value;
                    break;
				case "hd_end" :
					model.hd_end = (DateTime)value;
                    break;
				case "begin_time" :
				...
				case "hd_rate" :
                    ...
                    break;
				case "hd_type" :
					model.hd_type = (string)value;
                    break;
				case "hd_kind" :
					model.hd_kind = (int)value;
                    break;
				case "depart_id" :
				...
				case "alway_use" :
                    ...
                    break;
				case "sub_depart" :
					model.sub_depart = (short)value;
                    break;
				case "need_write" :
					model.need_write = (short)value;
                    break;
				case "have_hrs" :
        ...
        public int GetRecordCount()
        {
			...
			else
			{
				...
				return select.GetRecordCount<Holidays>();
			}
        }

		/// <summary>
		/// 獲?Holidays表記錄總數——從數據庫中查詢
		/// </summary>
        /// <param name="wheres">條件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionHelper.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<Holidays>(wheres);

		}

		/// <summary>
		/// 獲?Holidays表指定條件的記錄總數——從數據庫中查詢
		/// </summary>
        /// <param name="expression">條件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<Holidays, bool>> expression) {
            return new Select().From<Holidays>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.Holidays> Find(Expression<Func<DataAccess.Model.Holidays, bool>> expression)
        {
			//從緩存中獲?記錄集
			var list = GetList();
            //判斷獲?記錄集是否為null
        ...
        public bool Exist(int id)
        {
            ...
            //從數據庫中查找
            return Holidays.Exists(x => x.Id == id);
        }
        ...
        /// <param name="expression">條件語句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.Holidays, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionHelper.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分頁查詢
                var select = new SelectHelper();
                return select.SelectDataTable<Holidays>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //記錄?志
                CommonBll.WriteLog("獲?Holidays表記錄時出現異常", e);
		...
		#region 添加與編輯Holidays表記錄
		/// <summary>
		/// 添加與編輯Holidays記錄
		/// </summary>
	    /// <param name="page">當?頁面指針</param>
		/// <param name="model">Holidays表實伐</param>
        /// <param name="content">更新說明</param>
        /// <param name="isCache">是否更新緩存</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
        public void Save(Page page, Holidays model, string content = null, bool isCache = true, bool isAddUseLog = true)
        {
			try {
				//保存
				model.Save();
				...
				if (isAddUseLog)
				{
					...
					{
						content = "{0}" + (model.Id == 0 ? "添加" : "編輯") + "Holidays記錄成功，ID為【" + model.Id + "】";
					}
				...
				var result = "執行HolidaysBll.Save()函數出錯！";
		...
		{
			//設瞞Sql語句
			var sql = string.Format("delete from {0} where {1} = {2}", HolidaysTable.TableName,  HolidaysTable.Id, id);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了Holidays表id為【" + id + "】的記錄！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//設瞞Sql語句
			var sql = string.Format("delete from {0} where {1} in ({2})", HolidaysTable.TableName,  HolidaysTable.Id, str);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了Holidays表id為【" + str + "】的記錄！");
			}
		}

		/// <summary>
        /// 刪除Holidays表記錄——?果使用了緩存，刪除成功後會?空本表的所有緩存記錄，?後重新加載進緩存
        /// </summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="expression">條件語句</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
        public void Delete(Page page, Expression<Func<Holidays, bool>> expression, bool isAddUseLog = true)
        {
			//執行刪除
			Holidays.Delete(expression);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了Holidays表記錄！");
			}
        }
        ...
        {
            //設瞞Sql語句
            var sql = string.Format("delete from {0}", HolidaysTable.TableName);
            ...
            {
                //添加用戶操作記錄
                UseLogBll.GetInstence().Save(page, "{0}刪除了Holidays表所有記錄！");
            }
        }
		#endregion
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "Holidays", sortName, "Id"))
		    {
				//判斷是否?用緩存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}更新了Holidays表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "Holidays", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判斷是否?用緩存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}對Holidays表進行了自動排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
	        ...
	        else
	        {
				if (string.IsNullOrEmpty(conditionColName))
	            {
	                conditionColName = HolidaysTable.Id;
	            }
        ...
        /// <param name="expression">條件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.Holidays, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<Holidays>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.Holidays model, string colName)
		{
			...
			switch (colName)
			{
				case "Id" :
					return model.Id;
				case "hd_code" :
					return model.hd_code;
				case "hd_name" :
					return model.hd_name;
				case "hd_date" :
					return model.hd_date;
				case "hd_end" :
					return model.hd_end;
				case "begin_time" :
				...
				case "hd_type" :
					return model.hd_type;
				case "hd_kind" :
					return model.hd_kind;
				case "depart_id" :
					return model.depart_id;
				case "alway_use" :
					return model.alway_use;
				case "sub_depart" :
					return model.sub_depart;
				case "need_write" :
					return model.need_write;
				case "have_hrs" :
		...
		#region 更新Holidays表指定字段值
		/// <summary>更新Holidays表記錄指定字段值，?果使用了緩存，保存成功後會?空本表的所有緩存記錄，?後重新加載進緩存</summary>
		/// <param name="page">當?頁面指針</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionHelper.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<Holidays>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用戶操作記錄
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}囊改了Holidays表記錄。");				
				}
				else
				{
					//添加用戶操作記錄
		...
		#region 更新Holidays表指定主鍵Id的字段值
		/// <summary>更新Holidays表記錄指定字段值</summary>
        /// <param name="page">當?頁面指針</param>
        ...
        {
			content = content != "" ? content : "{0}囊改了Holidays表主鍵Id值為" + id + "的記錄。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionHelper.SqlqueryCondition(ConstraintType.And, HolidaysTable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新Holidays表記錄指定字段值（更新一個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了Holidays表主鍵Id值為" + id + "的記錄，將" + columnName + "字段值囊改為" + columnValue;
            //設瞞更新字段
		 ...
		 /// <summary>更新Holidays表記錄指定字段值（更新兩個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了Holidays表主鍵Id值為" + id + "的記錄，將" + columnName1 + "字段值囊改為" + columnValue1 + "，" + columnName2 + "字段值囊改為" + columnValue2;
            //設瞞更新字段
            var dic = new Dictionary<string, object>();
        ...
        /// <param name="isCache">是否從緩存中讀?</param>
        /// <returns></returns>
        public string Gethd_name(Page page, int pkValue, bool isCache = true)
        {
            //判斷是否?有用緩存
            if (isCache && CommonBll.IsUseCache())
            {
                ...
                return model == null ? "" : model.hd_name;
            }
            else
            {
                //從數據庫中查詢
                var model = Holidays.SingleOrDefault(x => x.Id == pkValue);
                return model == null ? "" : model.hd_name;
            }
        }
        #endregion
