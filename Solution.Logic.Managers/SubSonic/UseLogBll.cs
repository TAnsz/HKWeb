...
namespace Solution.Logic.Managers {
	...
	public partial class UseLogBll : LogicBase {
        ...
        public IList<DataAccess.Model.UseLog> GetList(bool isCache = true)
        {
            ...
            {
                //記錄?志
                CommonBll.WriteLog("從IIS緩存中獲取UseLog表記錄時出現異常", e);
			}
		...
		#region 實伐轉換
		/// <summary>
		/// 將UseLog記錄實伐（SubSonic實伐）轉換為?通的實伐（DataAccess.Model.UseLog）
		/// </summary>
		...
		/// <summary>
		/// 將UseLog記錄實伐集（SubSonic實伐）轉換為?通的實伐集（DataAccess.Model.UseLog）
		/// </summary>
		...
		/// <summary>
		/// 將UseLog記錄實伐批?通的實伐（DataAccess.Model.UseLog）轉換為SubSonic插件生成的實伐
		/// </summary>
        /// <param name="model">?通的實伐（DataAccess.Model.UseLog）</param>
		...
		/// <summary>
		/// 將UseLog記錄實伐批?通實伐集（DataAccess.Model.UseLog）轉換為SubSonic插件生成的實伐集
		/// </summary>
        /// <param name="sourceList">?通實伐集（DataAccess.Model.UseLog）</param>
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionHelper.SqlqueryCondition> wheres = null, List<string> sorts = null) {
            ...
            {
                //記錄?志
                CommonBll.WriteLog("獲?UseLog表記錄時出現異常", e);
		...
		#region 添加與編輯UseLog表記錄
		/// <summary>
		/// 添加與編輯UseLog記錄
		/// </summary>
	    /// <param name="page">當?頁面指針</param>
		/// <param name="model">UseLog表實伐</param>
        ...
        public void Save(Page page, UseLog model, string content = null, bool isCache = true, bool isAddUseLog = true)
        {
			try {
				...
				if (isAddUseLog)
				{
					...
					{
						content = "{0}" + (model.Id == 0 ? "添加" : "編輯") + "UseLog記錄成功，ID為【" + model.Id + "】";
					}
				...
				var result = "執行UseLogBll.Save()函數出錯！";
		...
		public override void Delete(Page page, int id, bool isAddUseLog = true) 
		{
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了UseLog表id為【" + id + "】的記錄！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了UseLog表id為【" + str + "】的記錄！");
			}
		}

		/// <summary>
        /// 刪除UseLog表記錄——?果使用了緩存，刪除成功後會?空本表的所有緩存記錄，?後重新加載進緩存
        /// </summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="expression">條件語句</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
        public void Delete(Page page, Expression<Func<UseLog, bool>> expression, bool isAddUseLog = true)
        {
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了UseLog表記錄！");
			}
        }
        ...
        public void DeleteAll(Page page, bool isAddUseLog = true)
        {
            ...
            {
                //添加用戶操作記錄
                UseLogBll.GetInstence().Save(page, "{0}刪除了UseLog表所有記錄！");
            }
        }
		#endregion
		...
		public override bool UpdateAutoSort(Page page, string strWhere = "", bool isExistsMoreLv = false, int pid = 0, string fieldName = "Sort", string fieldParentId = "ParentId")
	    {
			...
			if (CommonBll.AutoSort("Id", "UseLog", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
			    ...
			    //添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}對UseLog表進行了自動排序操作！");
		...
		/// <summary>更新UseLog表記錄指定字段值，?果使用了緩存，保存成功後會?空本表的所有緩存記錄，?後重新加載進緩存</summary>
		/// <param name="page">當?頁面指針</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionHelper.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			if (isAddUseLog){
				...
				{
					//添加用戶操作記錄
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}囊改了UseLog表記錄。");				
				}
				else
				{
					//添加用戶操作記錄
		...
		#region 更新UseLog表指定主鍵Id的字段值
		/// <summary>更新UseLog表記錄指定字段值</summary>
        /// <param name="page">當?頁面指針</param>
        ...
        {
			content = content != "" ? content : "{0}囊改了UseLog表主鍵Id值為" + id + "的記錄。";
        ...
        /// <summary>更新UseLog表記錄指定字段值（更新一個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了UseLog表主鍵Id值為" + id + "的記錄，將" + columnName + "字段值囊改為" + columnValue;
            //設瞞更新字段
		 ...
		 /// <summary>更新UseLog表記錄指定字段值（更新兩個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了UseLog表主鍵Id值為" + id + "的記錄，將" + columnName1 + "字段值囊改為" + columnValue1 + "，" + columnName2 + "字段值囊改為" + columnValue2;
            //設瞞更新字段
            var dic = new Dictionary<string, object>();
		...
		#region 刪除UseLog表指定MenuInfo_Id的字段值記錄
		/// <summary>
		/// 刪除UseLog表指定MenuInfo_Id的字段值記錄
		/// </summary>
		/// <param name="page">當?頁面指針</param>
		/// <param name="id">記錄的主鍵值</param>
		public void DeleteByMenuInfo_Id(Page page, int id) {
			...
			//添加用戶操作記錄
			UseLogBll.GetInstence().Save(page, "{0}刪除了UseLog表MenuInfo_Id值為【" + id + "】的記錄！");
		}
		...
		public void DeleteByMenuInfo_Id(Page page, int[] id) {
			...
			//添加用戶操作記錄
			UseLogBll.GetInstence().Save(page, "{0}刪除了UseLog表MenuInfo_Id值為【" + str + "】的記錄！");
		}
		#endregion
        ...
        {
			content = content != "" ? content : "{0}囊改了UseLog表外鍵MenuInfo_Id值為" + MenuInfo_Id + "的所有記錄。";
        ...
        {
			content = content != "" ? content : "{0}囊改了UseLog表外鍵MenuInfo_Id值為" + MenuInfo_Id + "的所有記錄，將" + columnName + "字段值囊改為" + columnValue;
            //設瞞更新字段
            var dic = new Dictionary<string, object>();
            dic.Add(columnName, columnValue);
        ...
        {
			content = content != "" ? content : "{0}囊改了UseLog表外鍵MenuInfo_Id值為" + MenuInfo_Id + "的所有記錄，將" + columnName1 + "字段值囊改為" + columnValue1 + "，" + columnName2 + "字段值囊改為" + columnValue2;
            //設瞞更新字段
            var dic = new Dictionary<string, object>();
            dic.Add(columnName1, columnValue1);
            dic.Add(columnName2, columnValue2);
