...
namespace Solution.Logic.Managers {
	...
	/// ANNUALLEAVE表邏輯類
	/// </summary>
	public partial class ANNUALLEAVEBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_ANNUALLEAVE";
        private const string const_CacheKey_Date = "Cache_ANNUALLEAVE_Date";

		#region 單例模式
		//定義單例實伐
		private static ANNUALLEAVEBll _ANNUALLEAVEBll = null;

		/// <summary>
		/// 獲取本邏輯類單例
		/// </summary>
		/// <returns></returns>
		public static ANNUALLEAVEBll GetInstence() {
			if (_ANNUALLEAVEBll == null) {
				_ANNUALLEAVEBll = new ANNUALLEAVEBll();
			}
			return _ANNUALLEAVEBll;
		}
		#endregion
		...
		#region 從IIS緩存中獲取ANNUALLEAVE表記錄
		/// <summary>
        /// 從IIS緩存中獲取ANNUALLEAVE表記錄
        /// </summary>
	    /// <param name="isCache">是否從緩存中讀取</param>
        public IList<DataAccess.Model.ANNUALLEAVE> GetList(bool isCache = true)
        {
			try
			{
				//判斷是否使用緩存
				if (CommonBll.IsUseCache() && isCache){
					...
					{
						return (IList<DataAccess.Model.ANNUALLEAVE>)obj;
					}
				}
				else
				{
					//定義臨時實伐集
					IList<DataAccess.Model.ANNUALLEAVE> list = null;

					//獲??表緩存加載條件表達式
					var exp = GetExpression<ANNUALLEAVE>();
                    //?果條件為空，則查詢?表所有記錄
					if (exp == null)
					{
						//從數據庫中獲?所有記錄
						var all = ANNUALLEAVE.All();
                        list = all == null ? null : Transform(all.ToList());
					}
					else
					{
                        //從數據庫中查詢出指定條件的記錄，並轉換為指定實伐集
						var all = ANNUALLEAVE.Find(exp);
            ...
            {
                //記錄?志
                CommonBll.WriteLog("從IIS緩存中獲取ANNUALLEAVE表記錄時出現異常", e);
			}
		...
		/// <returns>DataAccess.Model.ANNUALLEAVE</returns>
        public DataAccess.Model.ANNUALLEAVE GetModel(long id, bool isCache = true)
        {
            //判斷是否使用緩存
		    if (CommonBll.IsUseCache() && isCache)
		    {
		    ...
		    {
                //從數據庫中直接讀?
                var model = ANNUALLEAVE.SingleOrDefault(x => x.Id == id);
                if (model == null)
                {
                    return null;
                }
        ...
        /// <param name="id">主鍵Id</param>
		/// <returns>DataAccess.Model.ANNUALLEAVE</returns>
        public DataAccess.Model.ANNUALLEAVE GetModelForCache(long id)
        {
			try
			{
				//從緩存中讀?指定Id記錄
				...
				if (model == null){
					//從數據庫中讀?
					var tem = ANNUALLEAVE.SingleOrDefault(x => x.Id == id);
					if (tem == null)
					{
						return null;
					}
        ...
        /// <returns>DataAccess.Model.ANNUALLEAVE</returns>
        public DataAccess.Model.ANNUALLEAVE GetModelForCache(string conditionColName, object value)
        {
			try
            {
                //從緩存中獲取List
                var list = GetList();
                DataAccess.Model.ANNUALLEAVE model = null;
                Expression<Func<ANNUALLEAVE, bool>> expression = null;
                ...
                switch (conditionColName)
                {
					case "EMP_ID" :
                        ...
                        break;
					case "YEARS" :
						model = list.SingleOrDefault(x => x.YEARS == (int)value);
                        expression = x => x.YEARS == (int)value;
                        break;
					case "DATA" :
						model = list.SingleOrDefault(x => x.DATA == (decimal)value);
                        expression = x => x.DATA == (decimal)value;
                        break;
					case "MEMO" :
                        ...
                        break;
					case "S_DATE" :
						model = list.SingleOrDefault(x => x.S_DATE == (DateTime)value);
                        expression = x => x.S_DATE == (DateTime)value;
                        break;
					case "EDIT_BY" :
					...
					case "EDIT_DATE" :
                        ...
                        break;
					case "IsValid" :
						model = list.SingleOrDefault(x => x.IsValid == (bool)value);
                        expression = x => x.IsValid == (bool)value;
                        break;
					case "Id" :
                ...
                {
                    //從數據庫中讀取
                    var tem = ANNUALLEAVE.SingleOrDefault(expression);
                    if (tem == null)
                    {
                        return null;
                    }
        ...
        /// <returns>DataAccess.Model.ANNUALLEAVE</returns>
        public DataAccess.Model.ANNUALLEAVE GetModelForCache(Expression<Func<DataAccess.Model.ANNUALLEAVE, bool>> expression)
        {
			//從緩存中讀?記錄列表
			var list = GetList();
            //?果條件為空，則查詢?表所有記錄
        ...
        public void SetModelForCache(DataAccess.Model.ANNUALLEAVE model)
        {
		    ...
		    if (list == null)
		    {
                list = new List<DataAccess.Model.ANNUALLEAVE>();
		    }
            //添加記錄
            list.Add(model);
        }
        ...
        /// </summary>
        /// <param name="model">記錄實伐</param>
        public void SetModelForCache(ANNUALLEAVE model)
        {
		...
		/// <summary>
        /// 按條件刪除IIS緩存中ANNUALLEAVE表的指定記錄
        /// </summary>
        /// <param name="expression">條件，值為null時刪除?有記錄</param>
		public void DelCache(Expression<Func<DataAccess.Model.ANNUALLEAVE, bool>> expression)
        {
            //從緩存中獲?List
		    var list = GetList();
            //?果緩存為null，則不做?何處理
		...
		#region 實伐轉換
		/// <summary>
		/// 將ANNUALLEAVE記錄實伐（SubSonic實伐）轉換為?通的實伐（DataAccess.Model.ANNUALLEAVE）
		/// </summary>
        /// <param name="model">SubSonic插件生成的實伐</param>
		/// <returns>DataAccess.Model.ANNUALLEAVE</returns>
		public DataAccess.Model.ANNUALLEAVE Transform(ANNUALLEAVE model)
        {			
			if (model == null) 
				return null;

            return new DataAccess.Model.ANNUALLEAVE
            {
                EMP_ID = model.EMP_ID,
                YEARS = model.YEARS,
                DATA = model.DATA,
		...
		/// <summary>
		/// 將ANNUALLEAVE記錄實伐集（SubSonic實伐）轉換為?通的實伐集（DataAccess.Model.ANNUALLEAVE）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的實伐集</param>
        public IList<DataAccess.Model.ANNUALLEAVE> Transform(IList<ANNUALLEAVE> sourceList)
        {
			//創建List??
            var list = new List<DataAccess.Model.ANNUALLEAVE>();
		...
		/// <summary>
		/// 將ANNUALLEAVE記錄實伐批?通的實伐（DataAccess.Model.ANNUALLEAVE）轉換為SubSonic插件生成的實伐
		/// </summary>
        /// <param name="model">?通的實伐（DataAccess.Model.ANNUALLEAVE）</param>
		/// <returns>ANNUALLEAVE</returns>
		public ANNUALLEAVE Transform(DataAccess.Model.ANNUALLEAVE model)
        {
			if (model == null) 
				return null;
		...
		/// <summary>
		/// 將ANNUALLEAVE記錄實伐批?通實伐集（DataAccess.Model.ANNUALLEAVE）轉換為SubSonic插件生成的實伐集
		/// </summary>
        /// <param name="sourceList">?通實伐集（DataAccess.Model.ANNUALLEAVE）</param>
        public IList<ANNUALLEAVE> Transform(IList<DataAccess.Model.ANNUALLEAVE> sourceList)
        {
			//創建List??
            var list = new List<ANNUALLEAVE>();
		...
		public void SetModelValue(DataAccess.Model.ANNUALLEAVE model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.ANNUALLEAVE model, string colName, object value)
		{
            ...
            switch (colName)
            {
				case "EMP_ID" :
                    ...
                    break;
				case "YEARS" :
					model.YEARS = (int)value;
                    break;
				case "DATA" :
					model.DATA = (decimal)value;
                    break;
				case "MEMO" :
					model.MEMO = (string)value;
                    break;
				case "S_DATE" :
					model.S_DATE = (DateTime)value;
                    break;
				case "EDIT_BY" :
        ...
        public int GetRecordCount()
        {
			...
			else
			{
				...
				return select.GetRecordCount<ANNUALLEAVE>();
			}
        }

		/// <summary>
		/// 獲?ANNUALLEAVE表記錄總數——從數據庫中查詢
		/// </summary>
        /// <param name="wheres">條件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionHelper.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<ANNUALLEAVE>(wheres);

		}

		/// <summary>
		/// 獲?ANNUALLEAVE表指定條件的記錄總數——從數據庫中查詢
		/// </summary>
        /// <param name="expression">條件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<ANNUALLEAVE, bool>> expression) {
            return new Select().From<ANNUALLEAVE>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.ANNUALLEAVE> Find(Expression<Func<DataAccess.Model.ANNUALLEAVE, bool>> expression)
        {
			//從緩存中獲?記錄集
			var list = GetList();
            //判斷獲?記錄集是否為null
        ...
        public bool Exist(int id)
        {
            ...
            //從數據庫中查找
            return ANNUALLEAVE.Exists(x => x.Id == id);
        }
        ...
        /// <param name="expression">條件語句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.ANNUALLEAVE, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionHelper.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分頁查詢
                var select = new SelectHelper();
                return select.SelectDataTable<ANNUALLEAVE>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //記錄?志
                CommonBll.WriteLog("獲?ANNUALLEAVE表記錄時出現異常", e);
		...
		#region 添加與編輯ANNUALLEAVE表記錄
		/// <summary>
		/// 添加與編輯ANNUALLEAVE記錄
		/// </summary>
	    /// <param name="page">當?頁面指針</param>
		/// <param name="model">ANNUALLEAVE表實伐</param>
        /// <param name="content">更新說明</param>
        /// <param name="isCache">是否更新緩存</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
        public void Save(Page page, ANNUALLEAVE model, string content = null, bool isCache = true, bool isAddUseLog = true)
        {
			try {
				//保存
				model.Save();
				...
				if (isAddUseLog)
				{
					...
					{
						content = "{0}" + (model.Id == 0 ? "添加" : "編輯") + "ANNUALLEAVE記錄成功，ID為【" + model.Id + "】";
					}
				...
				var result = "執行ANNUALLEAVEBll.Save()函數出錯！";
		...
		{
			//設瞞Sql語句
			var sql = string.Format("delete from {0} where {1} = {2}", ANNUALLEAVETable.TableName,  ANNUALLEAVETable.Id, id);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了ANNUALLEAVE表id為【" + id + "】的記錄！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//設瞞Sql語句
			var sql = string.Format("delete from {0} where {1} in ({2})", ANNUALLEAVETable.TableName,  ANNUALLEAVETable.Id, str);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了ANNUALLEAVE表id為【" + str + "】的記錄！");
			}
		}

		/// <summary>
        /// 刪除ANNUALLEAVE表記錄——?果使用了緩存，刪除成功後會?空本表的所有緩存記錄，?後重新加載進緩存
        /// </summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="expression">條件語句</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
        public void Delete(Page page, Expression<Func<ANNUALLEAVE, bool>> expression, bool isAddUseLog = true)
        {
			//執行刪除
			ANNUALLEAVE.Delete(expression);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了ANNUALLEAVE表記錄！");
			}
        }
        ...
        {
            //設瞞Sql語句
            var sql = string.Format("delete from {0}", ANNUALLEAVETable.TableName);
            ...
            {
                //添加用戶操作記錄
                UseLogBll.GetInstence().Save(page, "{0}刪除了ANNUALLEAVE表所有記錄！");
            }
        }
		#endregion
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "ANNUALLEAVE", sortName, "EMP_ID"))
		    {
				//判斷是否?用緩存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}更新了ANNUALLEAVE表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("EMP_ID", "ANNUALLEAVE", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判斷是否?用緩存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}對ANNUALLEAVE表進行了自動排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
	        ...
	        else
	        {
				if (string.IsNullOrEmpty(conditionColName))
	            {
	                conditionColName = ANNUALLEAVETable.Id;
	            }
        ...
        /// <param name="expression">條件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.ANNUALLEAVE, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<ANNUALLEAVE>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.ANNUALLEAVE model, string colName)
		{
		...
		#region 更新ANNUALLEAVE表指定字段值
		/// <summary>更新ANNUALLEAVE表記錄指定字段值，?果使用了緩存，保存成功後會?空本表的所有緩存記錄，?後重新加載進緩存</summary>
		/// <param name="page">當?頁面指針</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionHelper.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<ANNUALLEAVE>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用戶操作記錄
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}囊改了ANNUALLEAVE表記錄。");				
				}
				else
				{
					//添加用戶操作記錄
		...
		#region 更新ANNUALLEAVE表指定主鍵Id的字段值
		/// <summary>更新ANNUALLEAVE表記錄指定字段值</summary>
        /// <param name="page">當?頁面指針</param>
        ...
        {
			content = content != "" ? content : "{0}囊改了ANNUALLEAVE表主鍵Id值為" + id + "的記錄。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionHelper.SqlqueryCondition(ConstraintType.And, ANNUALLEAVETable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新ANNUALLEAVE表記錄指定字段值（更新一個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了ANNUALLEAVE表主鍵Id值為" + id + "的記錄，將" + columnName + "字段值囊改為" + columnValue;
            //設瞞更新字段
		 ...
		 /// <summary>更新ANNUALLEAVE表記錄指定字段值（更新兩個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了ANNUALLEAVE表主鍵Id值為" + id + "的記錄，將" + columnName1 + "字段值囊改為" + columnValue1 + "，" + columnName2 + "字段值囊改為" + columnValue2;
            //設瞞更新字段
            var dic = new Dictionary<string, object>();
