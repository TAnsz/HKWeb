...
namespace Solution.Logic.Managers {
	...
	public partial class LoginLogBll : LogicBase {
		...
		#region 從IIS緩存中獲取LoginLog表記錄
		/// <summary>
        /// 從IIS緩存中獲取LoginLog表記錄
        /// </summary>
        ...
        public IList<DataAccess.Model.LoginLog> GetList(bool isCache = true)
        {
            ...
            {
                //記錄?志
                CommonBll.WriteLog("從IIS緩存中獲取LoginLog表記錄時出現異常", e);
			}
		...
		#region 實伐轉換
		/// <summary>
		/// 將LoginLog記錄實伐（SubSonic實伐）轉換為?通的實伐（DataAccess.Model.LoginLog）
		/// </summary>
		...
		/// <summary>
		/// 將LoginLog記錄實伐集（SubSonic實伐）轉換為?通的實伐集（DataAccess.Model.LoginLog）
		/// </summary>
		...
		/// <summary>
		/// 將LoginLog記錄實伐批?通的實伐（DataAccess.Model.LoginLog）轉換為SubSonic插件生成的實伐
		/// </summary>
        /// <param name="model">?通的實伐（DataAccess.Model.LoginLog）</param>
		...
		/// <summary>
		/// 將LoginLog記錄實伐批?通實伐集（DataAccess.Model.LoginLog）轉換為SubSonic插件生成的實伐集
		/// </summary>
        /// <param name="sourceList">?通實伐集（DataAccess.Model.LoginLog）</param>
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionHelper.SqlqueryCondition> wheres = null, List<string> sorts = null) {
            ...
            {
                //記錄?志
                CommonBll.WriteLog("獲?LoginLog表記錄時出現異常", e);
		...
		#region 添加與編輯LoginLog表記錄
		/// <summary>
		/// 添加與編輯LoginLog記錄
		/// </summary>
	    /// <param name="page">當?頁面指針</param>
		/// <param name="model">LoginLog表實伐</param>
        ...
        public void Save(Page page, LoginLog model, string content = null, bool isCache = true, bool isAddUseLog = true)
        {
			try {
				...
				if (isAddUseLog)
				{
					...
					{
						content = "{0}" + (model.Id == 0 ? "添加" : "編輯") + "LoginLog記錄成功，ID為【" + model.Id + "】";
					}
				...
				var result = "執行LoginLogBll.Save()函數出錯！";
		...
		public override void Delete(Page page, int id, bool isAddUseLog = true) 
		{
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了LoginLog表id為【" + id + "】的記錄！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了LoginLog表id為【" + str + "】的記錄！");
			}
		}

		/// <summary>
        /// 刪除LoginLog表記錄——?果使用了緩存，刪除成功後會?空本表的所有緩存記錄，?後重新加載進緩存
        /// </summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="expression">條件語句</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
        public void Delete(Page page, Expression<Func<LoginLog, bool>> expression, bool isAddUseLog = true)
        {
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了LoginLog表記錄！");
			}
        }
        ...
        public void DeleteAll(Page page, bool isAddUseLog = true)
        {
            ...
            {
                //添加用戶操作記錄
                UseLogBll.GetInstence().Save(page, "{0}刪除了LoginLog表所有記錄！");
            }
        }
		#endregion
		...
		public override bool UpdateAutoSort(Page page, string strWhere = "", bool isExistsMoreLv = false, int pid = 0, string fieldName = "Sort", string fieldParentId = "ParentId")
	    {
			...
			if (CommonBll.AutoSort("Id", "LoginLog", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
			    ...
			    //添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}對LoginLog表進行了自動排序操作！");
		...
		/// <summary>更新LoginLog表記錄指定字段值，?果使用了緩存，保存成功後會?空本表的所有緩存記錄，?後重新加載進緩存</summary>
		/// <param name="page">當?頁面指針</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionHelper.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			if (isAddUseLog){
				...
				{
					//添加用戶操作記錄
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}囊改了LoginLog表記錄。");				
				}
				else
				{
					//添加用戶操作記錄
		...
		#region 更新LoginLog表指定主鍵Id的字段值
		/// <summary>更新LoginLog表記錄指定字段值</summary>
        /// <param name="page">當?頁面指針</param>
        ...
        {
			content = content != "" ? content : "{0}囊改了LoginLog表主鍵Id值為" + id + "的記錄。";
        ...
        /// <summary>更新LoginLog表記錄指定字段值（更新一個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了LoginLog表主鍵Id值為" + id + "的記錄，將" + columnName + "字段值囊改為" + columnValue;
            //設瞞更新字段
		 ...
		 /// <summary>更新LoginLog表記錄指定字段值（更新兩個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了LoginLog表主鍵Id值為" + id + "的記錄，將" + columnName1 + "字段值囊改為" + columnValue1 + "，" + columnName2 + "字段值囊改為" + columnValue2;
            //設瞞更新字段
            var dic = new Dictionary<string, object>();
