...
namespace Solution.Logic.Managers {
	...
	/// TOOL_LIST表邏輯類
	/// </summary>
	public partial class TOOL_LISTBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_TOOL_LIST";
        private const string const_CacheKey_Date = "Cache_TOOL_LIST_Date";

		#region 單例模式
		//定義單例實伐
		private static TOOL_LISTBll _TOOL_LISTBll = null;

		/// <summary>
		/// 獲取本邏輯類單例
		/// </summary>
		/// <returns></returns>
		public static TOOL_LISTBll GetInstence() {
			if (_TOOL_LISTBll == null) {
				_TOOL_LISTBll = new TOOL_LISTBll();
			}
			return _TOOL_LISTBll;
		}
		#endregion
		...
		#region 從IIS緩存中獲取TOOL_LIST表記錄
		/// <summary>
        /// 從IIS緩存中獲取TOOL_LIST表記錄
        /// </summary>
	    /// <param name="isCache">是否從緩存中讀取</param>
        public IList<DataAccess.Model.TOOL_LIST> GetList(bool isCache = true)
        {
			try
			{
				//判斷是否使用緩存
				if (CommonBll.IsUseCache() && isCache){
					...
					{
						return (IList<DataAccess.Model.TOOL_LIST>)obj;
					}
				}
				else
				{
					//定義臨時實伐集
					IList<DataAccess.Model.TOOL_LIST> list = null;

					//獲??表緩存加載條件表達式
					var exp = GetExpression<TOOL_LIST>();
                    //?果條件為空，則查詢?表所有記錄
					if (exp == null)
					{
						//從數據庫中獲?所有記錄
						var all = TOOL_LIST.All();
                        list = all == null ? null : Transform(all.ToList());
					}
					else
					{
                        //從數據庫中查詢出指定條件的記錄，並轉換為指定實伐集
						var all = TOOL_LIST.Find(exp);
            ...
            {
                //記錄?志
                CommonBll.WriteLog("從IIS緩存中獲取TOOL_LIST表記錄時出現異常", e);
			}
		...
		/// <returns>DataAccess.Model.TOOL_LIST</returns>
        public DataAccess.Model.TOOL_LIST GetModel(long id, bool isCache = true)
        {
            //判斷是否使用緩存
		    if (CommonBll.IsUseCache() && isCache)
		    {
		    ...
		    {
                //從數據庫中直接讀?
                var model = TOOL_LIST.SingleOrDefault(x => x.Id == id);
                if (model == null)
                {
                    return null;
                }
        ...
        /// <param name="id">主鍵Id</param>
		/// <returns>DataAccess.Model.TOOL_LIST</returns>
        public DataAccess.Model.TOOL_LIST GetModelForCache(long id)
        {
			try
			{
				//從緩存中讀?指定Id記錄
				...
				if (model == null){
					//從數據庫中讀?
					var tem = TOOL_LIST.SingleOrDefault(x => x.Id == id);
					if (tem == null)
					{
						return null;
					}
        ...
        /// <returns>DataAccess.Model.TOOL_LIST</returns>
        public DataAccess.Model.TOOL_LIST GetModelForCache(string conditionColName, object value)
        {
			try
            {
                //從緩存中獲取List
                var list = GetList();
                DataAccess.Model.TOOL_LIST model = null;
                Expression<Func<TOOL_LIST, bool>> expression = null;
                ...
                switch (conditionColName)
                {
					case "Id" :
                        ...
                        break;
					case "TOOL_NO" :
						model = list.SingleOrDefault(x => x.TOOL_NO == (string)value);
                        expression = x => x.TOOL_NO == (string)value;
                        break;
					case "TEXT" :
						model = list.SingleOrDefault(x => x.TEXT == (string)value);
                        expression = x => x.TEXT == (string)value;
                        break;
					case "TOOLTIP" :
						model = list.SingleOrDefault(x => x.TOOLTIP == (string)value);
                        expression = x => x.TOOLTIP == (string)value;
                        break;
					case "IMAGE" :
						model = list.SingleOrDefault(x => x.IMAGE == (string)value);
                        expression = x => x.IMAGE == (string)value;
                        break;
					case "DIVIDER" :
						model = list.SingleOrDefault(x => x.DIVIDER == (int)value);
                        expression = x => x.DIVIDER == (int)value;
                        break;
					case "DISABLED" :
						model = list.SingleOrDefault(x => x.DISABLED == (int)value);
                        expression = x => x.DISABLED == (int)value;
                        break;
					case "DROPDOWN" :
						model = list.SingleOrDefault(x => x.DROPDOWN == (int)value);
                        expression = x => x.DROPDOWN == (int)value;
                        break;
					case "DROPWHOLE" :
						model = list.SingleOrDefault(x => x.DROPWHOLE == (int)value);
                        expression = x => x.DROPWHOLE == (int)value;
                        break;
					case "TAGDATA" :
						model = list.SingleOrDefault(x => x.TAGDATA == (string)value);
                        expression = x => x.TAGDATA == (string)value;
                        break;
					case "SORT_ORDER" :
						model = list.SingleOrDefault(x => x.SORT_ORDER == (int)value);
                        expression = x => x.SORT_ORDER == (int)value;
                        break;
					case "DWOBJECT" :
						model = list.SingleOrDefault(x => x.DWOBJECT == (string)value);
                        expression = x => x.DWOBJECT == (string)value;
                        break;
					case "GROUPNO" :
						model = list.SingleOrDefault(x => x.GROUPNO == (int)value);
                        expression = x => x.GROUPNO == (int)value;
                        break;
					case "GROUPNAME" :
						model = list.SingleOrDefault(x => x.GROUPNAME == (string)value);
                        expression = x => x.GROUPNAME == (string)value;
                        break;
                ...
                {
                    //從數據庫中讀取
                    var tem = TOOL_LIST.SingleOrDefault(expression);
                    if (tem == null)
                    {
                        return null;
                    }
        ...
        /// <returns>DataAccess.Model.TOOL_LIST</returns>
        public DataAccess.Model.TOOL_LIST GetModelForCache(Expression<Func<DataAccess.Model.TOOL_LIST, bool>> expression)
        {
			//從緩存中讀?記錄列表
			var list = GetList();
            //?果條件為空，則查詢?表所有記錄
        ...
        public void SetModelForCache(DataAccess.Model.TOOL_LIST model)
        {
		    ...
		    if (list == null)
		    {
                list = new List<DataAccess.Model.TOOL_LIST>();
		    }
            //添加記錄
            list.Add(model);
        }
        ...
        /// </summary>
        /// <param name="model">記錄實伐</param>
        public void SetModelForCache(TOOL_LIST model)
        {
		...
		/// <summary>
        /// 按條件刪除IIS緩存中TOOL_LIST表的指定記錄
        /// </summary>
        /// <param name="expression">條件，值為null時刪除?有記錄</param>
		public void DelCache(Expression<Func<DataAccess.Model.TOOL_LIST, bool>> expression)
        {
            //從緩存中獲?List
		    var list = GetList();
            //?果緩存為null，則不做?何處理
		...
		#region 實伐轉換
		/// <summary>
		/// 將TOOL_LIST記錄實伐（SubSonic實伐）轉換為?通的實伐（DataAccess.Model.TOOL_LIST）
		/// </summary>
        /// <param name="model">SubSonic插件生成的實伐</param>
		/// <returns>DataAccess.Model.TOOL_LIST</returns>
		public DataAccess.Model.TOOL_LIST Transform(TOOL_LIST model)
        {			
			if (model == null) 
				return null;

            return new DataAccess.Model.TOOL_LIST
            {
                Id = model.Id,
                TOOL_NO = model.TOOL_NO,
                TEXT = model.TEXT,
                ...
                DROPWHOLE = model.DROPWHOLE,
                TAGDATA = model.TAGDATA,
                SORT_ORDER = model.SORT_ORDER,
                DWOBJECT = model.DWOBJECT,
		...
		/// <summary>
		/// 將TOOL_LIST記錄實伐集（SubSonic實伐）轉換為?通的實伐集（DataAccess.Model.TOOL_LIST）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的實伐集</param>
        public IList<DataAccess.Model.TOOL_LIST> Transform(IList<TOOL_LIST> sourceList)
        {
			//創建List??
            var list = new List<DataAccess.Model.TOOL_LIST>();
		...
		/// <summary>
		/// 將TOOL_LIST記錄實伐批?通的實伐（DataAccess.Model.TOOL_LIST）轉換為SubSonic插件生成的實伐
		/// </summary>
        /// <param name="model">?通的實伐（DataAccess.Model.TOOL_LIST）</param>
		/// <returns>TOOL_LIST</returns>
		public TOOL_LIST Transform(DataAccess.Model.TOOL_LIST model)
        {
			if (model == null) 
				return null;

            return new TOOL_LIST
            {
                Id = model.Id,
                TOOL_NO = model.TOOL_NO,
                TEXT = model.TEXT,
		...
		/// <summary>
		/// 將TOOL_LIST記錄實伐批?通實伐集（DataAccess.Model.TOOL_LIST）轉換為SubSonic插件生成的實伐集
		/// </summary>
        /// <param name="sourceList">?通實伐集（DataAccess.Model.TOOL_LIST）</param>
        public IList<TOOL_LIST> Transform(IList<DataAccess.Model.TOOL_LIST> sourceList)
        {
			//創建List??
            var list = new List<TOOL_LIST>();
		...
		public void SetModelValue(DataAccess.Model.TOOL_LIST model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.TOOL_LIST model, string colName, object value)
		{
            ...
            switch (colName)
            {
				case "Id" :
                    ...
                    break;
				case "TOOL_NO" :
					model.TOOL_NO = (string)value;
                    break;
				case "TEXT" :
					model.TEXT = (string)value;
                    break;
				case "TOOLTIP" :
					model.TOOLTIP = (string)value;
                    break;
				case "IMAGE" :
					model.IMAGE = (string)value;
                    break;
				case "DIVIDER" :
					model.DIVIDER = (int)value;
                    break;
				case "DISABLED" :
					model.DISABLED = (int)value;
                    break;
				case "DROPDOWN" :
					model.DROPDOWN = (int)value;
                    break;
				case "DROPWHOLE" :
					model.DROPWHOLE = (int)value;
                    break;
				case "TAGDATA" :
					model.TAGDATA = (string)value;
                    break;
				case "SORT_ORDER" :
					model.SORT_ORDER = (int)value;
                    break;
				case "DWOBJECT" :
					model.DWOBJECT = (string)value;
                    break;
				case "GROUPNO" :
					model.GROUPNO = (int)value;
                    break;
				case "GROUPNAME" :
					model.GROUPNAME = (string)value;
                    break;
            }
		}
        ...
        public int GetRecordCount()
        {
			...
			else
			{
				...
				return select.GetRecordCount<TOOL_LIST>();
			}
        }

		/// <summary>
		/// 獲?TOOL_LIST表記錄總數——從數據庫中查詢
		/// </summary>
        /// <param name="wheres">條件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionHelper.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<TOOL_LIST>(wheres);

		}

		/// <summary>
		/// 獲?TOOL_LIST表指定條件的記錄總數——從數據庫中查詢
		/// </summary>
        /// <param name="expression">條件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<TOOL_LIST, bool>> expression) {
            return new Select().From<TOOL_LIST>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.TOOL_LIST> Find(Expression<Func<DataAccess.Model.TOOL_LIST, bool>> expression)
        {
			//從緩存中獲?記錄集
			var list = GetList();
            //判斷獲?記錄集是否為null
        ...
        public bool Exist(int id)
        {
            ...
            //從數據庫中查找
            return TOOL_LIST.Exists(x => x.Id == id);
        }
        ...
        /// <param name="expression">條件語句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.TOOL_LIST, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionHelper.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分頁查詢
                var select = new SelectHelper();
                return select.SelectDataTable<TOOL_LIST>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //記錄?志
                CommonBll.WriteLog("獲?TOOL_LIST表記錄時出現異常", e);
		...
		#region 添加與編輯TOOL_LIST表記錄
		/// <summary>
		/// 添加與編輯TOOL_LIST記錄
		/// </summary>
	    /// <param name="page">當?頁面指針</param>
		/// <param name="model">TOOL_LIST表實伐</param>
        /// <param name="content">更新說明</param>
        /// <param name="isCache">是否更新緩存</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
        public void Save(Page page, TOOL_LIST model, string content = null, bool isCache = true, bool isAddUseLog = true)
        {
			try {
				//保存
				model.Save();
				...
				if (isAddUseLog)
				{
					...
					{
						content = "{0}" + (model.Id == 0 ? "添加" : "編輯") + "TOOL_LIST記錄成功，ID為【" + model.Id + "】";
					}
				...
				var result = "執行TOOL_LISTBll.Save()函數出錯！";
		...
		{
			//設瞞Sql語句
			var sql = string.Format("delete from {0} where {1} = {2}", TOOL_LISTTable.TableName,  TOOL_LISTTable.Id, id);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了TOOL_LIST表id為【" + id + "】的記錄！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//設瞞Sql語句
			var sql = string.Format("delete from {0} where {1} in ({2})", TOOL_LISTTable.TableName,  TOOL_LISTTable.Id, str);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了TOOL_LIST表id為【" + str + "】的記錄！");
			}
		}

		/// <summary>
        /// 刪除TOOL_LIST表記錄——?果使用了緩存，刪除成功後會?空本表的所有緩存記錄，?後重新加載進緩存
        /// </summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="expression">條件語句</param>
		/// <param name="isAddUseLog">是否添加用戶操作?志</param>
        public void Delete(Page page, Expression<Func<TOOL_LIST, bool>> expression, bool isAddUseLog = true)
        {
			//執行刪除
			TOOL_LIST.Delete(expression);
		    ...
		    {
				//添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}刪除了TOOL_LIST表記錄！");
			}
        }
        ...
        {
            //設瞞Sql語句
            var sql = string.Format("delete from {0}", TOOL_LISTTable.TableName);
            ...
            {
                //添加用戶操作記錄
                UseLogBll.GetInstence().Save(page, "{0}刪除了TOOL_LIST表所有記錄！");
            }
        }
		#endregion
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "TOOL_LIST", sortName, "Id"))
		    {
				//判斷是否?用緩存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}更新了TOOL_LIST表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "TOOL_LIST", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判斷是否?用緩存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用戶操作記錄
				UseLogBll.GetInstence().Save(page, "{0}對TOOL_LIST表進行了自動排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
	        ...
	        else
	        {
				if (string.IsNullOrEmpty(conditionColName))
	            {
	                conditionColName = TOOL_LISTTable.Id;
	            }
        ...
        /// <param name="expression">條件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.TOOL_LIST, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<TOOL_LIST>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.TOOL_LIST model, string colName)
		{
			...
			switch (colName)
			{
				...
				case "DROPWHOLE" :
					return model.DROPWHOLE;
				case "TAGDATA" :
					return model.TAGDATA;
				case "SORT_ORDER" :
					return model.SORT_ORDER;
				case "DWOBJECT" :
					return model.DWOBJECT;
				case "GROUPNO" :
		...
		#region 更新TOOL_LIST表指定字段值
		/// <summary>更新TOOL_LIST表記錄指定字段值，?果使用了緩存，保存成功後會?空本表的所有緩存記錄，?後重新加載進緩存</summary>
		/// <param name="page">當?頁面指針</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionHelper.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<TOOL_LIST>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用戶操作記錄
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}囊改了TOOL_LIST表記錄。");				
				}
				else
				{
					//添加用戶操作記錄
		...
		#region 更新TOOL_LIST表指定主鍵Id的字段值
		/// <summary>更新TOOL_LIST表記錄指定字段值</summary>
        /// <param name="page">當?頁面指針</param>
        ...
        {
			content = content != "" ? content : "{0}囊改了TOOL_LIST表主鍵Id值為" + id + "的記錄。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionHelper.SqlqueryCondition(ConstraintType.And, TOOL_LISTTable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新TOOL_LIST表記錄指定字段值（更新一個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了TOOL_LIST表主鍵Id值為" + id + "的記錄，將" + columnName + "字段值囊改為" + columnValue;
            //設瞞更新字段
		 ...
		 /// <summary>更新TOOL_LIST表記錄指定字段值（更新兩個字段值）</summary>
        /// <param name="page">當?頁面指針</param>
        /// <param name="id">主鍵Id，當小於等於0時，則更新所有記錄</param>
        ...
        {
            content = content != "" ? content : "{0}囊改了TOOL_LIST表主鍵Id值為" + id + "的記錄，將" + columnName1 + "字段值囊改為" + columnValue1 + "，" + columnName2 + "字段值囊改為" + columnValue2;
            //設瞞更新字段
            var dic = new Dictionary<string, object>();
        ...
        /// <param name="isCache">是否從緩存中讀?</param>
        /// <returns></returns>
        public string GetGROUPNAME(Page page, int pkValue, bool isCache = true)
        {
            //判斷是否?有用緩存
            if (isCache && CommonBll.IsUseCache())
            {
                ...
                return model == null ? "" : model.GROUPNAME;
            }
            else
            {
                //從數據庫中查詢
                var model = TOOL_LIST.SingleOrDefault(x => x.Id == pkValue);
                return model == null ? "" : model.GROUPNAME;
            }
        }
        #endregion
