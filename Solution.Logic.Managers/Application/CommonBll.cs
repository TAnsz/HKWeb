...
namespace Solution.Logic.Managers
{
    ...
    public class CommonBll
    {
        ...
        #region 获取时段显示值
        public static string GetWorkType(string id)
        {
            switch (id)
            {
                case "0":
        ...
        public static string GetTableSN(string tbl, string col)
        {
            var sql = string.Format("select max({0}) from {1}", col, tbl);
            var select = new SelectHelper();
            var ret = select.ExecuteScalar(sql);
            return GetSerialNumber(ret);
        }
        /// <summary>
        /// 生成充值流水号格式：8位日期加4位顺序号，如201003020056。
        /// </summary>
        public static string GetSerialNumber(object serialNumber)
        {
            string def = DateTime.Now.ToString("yyyyMMdd") + "0001";
            if (serialNumber != null)
            {
                try
                {
                    string sn = serialNumber.ToString();
                    if (!(string.IsNullOrEmpty(sn) || sn.Length < 12))
                    {
                        string headDate = sn.Substring(0, 8);
                        int lastNumber = int.Parse(sn.Substring(8));
